...
class OpenFDAClient:
    ...
    def fetch_all(
        ...
        """Retrieve labels matching ``search``, optionally to a bounded limit.
        ...
        ``sort=set_id:asc`` and returned in that order. ``max_records`` bounds
        retrieval work while preserving the authoritative total count.
        """
        size = page_size or self.page_size
        if max_records is not None and (isinstance(max_records, bool) or max_records < 1):
        ...
        while True:
            ...
            if max_records is not None:
                request_size = min(request_size, max_records - len(records))
                if request_size <= 0:
                    break
            body = self._request(
                {"search": search, "limit": request_size, "skip": skip, "sort": "set_id:asc"}
            )
            if body.get("error", {}).get("code") == "NOT_FOUND":
            ...
            records.extend(page)
            skip += len(page)
            if skip >= total or not page or (max_records is not None and len(records) >= max_records):
                break
            if skip > MAX_SKIP:
