...
vi.mock('@/renderer/services/skills/skillsCatalog', () => ({
  loadAvailableSkillsWithSynonBiomed: vi.fn(async () => []),
}));
...
describe('useAssistantEditor', () => {
  ...
  it('optimistically updates and revalidates the shared assistant list when toggling enabled', async () => {
    ...
    expect(swrMutate).toHaveBeenNthCalledWith(1, 'assistants.list', expect.any(Function), {
      revalidate: false,
    });
    expect(ipcBridge.assistants.setState.invoke).toHaveBeenCalledWith({
      id: 'builtin-1',
      enabled: false,
    });
    expect(loadAssistantsMock).toHaveBeenCalled();
