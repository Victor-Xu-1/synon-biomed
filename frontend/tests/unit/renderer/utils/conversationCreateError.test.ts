...
describe('conversationCreateError', () => {
  ...
  it('uses a localized generic message for unrelated backend errors', () => {
    ...
    expect(getConversationCreateErrorMessage(error, t)).toBe('Failed to create conversation');
  });

  it('uses a localized generic message when workspace details are missing', () => {
    const error = httpError('WORKSPACE_PATH_UNAVAILABLE', 'Bad request: Workspace path contains whitespace');
  ...
  it('extracts backend payloads from stringified BackendHttpError messages', () => {
    const error =
      'Backend POST /api/conversations failed (400): {"success":false,"error":"Workspace path is unavailable: /Users/zhoukai/Documents/Archive . Make sure the selected workspace path exists and is accessible.","code":"WORKSPACE_PATH_UNAVAILABLE","details":{"workspace_path":"/Users/zhoukai/Documents/Archive ","operation":"create"}}';
