...
missing AF2/RF3 weights before the first real pipeline run. Build readiness
from the admitted compute snapshot, a bounded `nvidia-smi`/disk probe,
`complexa download --status`, and the exact config validation; no shared
preflight helper is bundled in this Skill.
