 ...
 *
 * Logo 真值由后端 `/api/synonbiomed/experts` 提供（投影自 agent_metadata.icon/avatar）。
 ...
 *
 * Logo truth lives in the backend (`/api/synonbiomed/experts`, projected from
...
function collectManagedAgentLogoKeys(agent: ManagedAgent): string[] {
  const keys = [agent.backend, agent.agent_type, agent.id];
  return keys
