...
const SpeechTestPanel: React.FC<SpeechTestPanelProps> = ({ config, source }) => {
  ...
  const buttonLabel =
    isRecording || isPreparing || isTranscribing ? t('settings.speechToTextTestStop') : t('settings.speechToTextTest');
  const statusHint = isRecording
    ? t('settings.speechToTextTestRecording')
    : isPreparing
      ? t('settings.speechToTextTestPreparingLocal')
      : isTranscribing
    ...
    <div className='mt-16px flex flex-col gap-8px'>
      ...
      <div className='flex items-center gap-12px'>
        <Button
          type='outline'
          shape='round'
          loading={isPreparing || isTranscribing}
