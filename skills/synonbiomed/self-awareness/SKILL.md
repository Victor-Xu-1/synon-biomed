---
name: self-awareness
description: Synon Biomed's own session database schema and SDK surface for introspection via host.query(). Load this when you need to query your own conversation history, token usage, cost accounting, execution log, or artifact metadata beyond what host.frames()/host.artifacts() provide — e.g. "how many tokens has this session used", "what was my last tool call", "list every file I've written", "where are messages stored", "what tables can I query", "inspect frames.context_data", or any time you're about to PRAGMA-probe the Synon Biomed metadata DB to discover its schema.
license: Apache-2.0
---

# Self-awareness — Synon Biomed's own database and SDK

`host.query(sql, params=[], limit=None, df=False)` runs read-only SQLite
against Synon Biomed's own metadata DB. It is only available via the **`repl`
...
- Secrets (encrypted at rest, blocked defense-in-depth): `oauth_tokens`,
  `user_secrets`, `synon_llm_api_keys`, `cloud_credentials`. →
...
| `host.delegate(task_or_list, name=?, profile=?, output_schema=?, model=?)` | **`repl`** | Spawn child agent(s), block until done (ultra-mode roots; requires `[delegation] sdk_enabled`). `model=` pins the child's model per request — e.g. a lighter active-profile id for cheap fan-outs. Blocks the cell — for long-running children run it in a background cell (a user message mid-call backgrounds it; a Stop / cell interrupt cancels the children) |
...
| `host.llm(prompt_or_list, model=?, ...)` | `python` | Single-turn completion via the host's API client. Omitting `model=` uses the active third-party LLM profile (via `[llm] kernel_default_model`); for harder reasoning pass `model=host.current_model()` — never hardcode a literal model id (they go stale) |
