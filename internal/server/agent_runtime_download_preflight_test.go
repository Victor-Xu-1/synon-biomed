...
func TestAgentRuntimeBashDownloadPreflightRoutesFileTransferClientsToDurableAuthority(t *testing.T) {
	for _, command := range []string{
		...
		preflight := agentRuntimeBashDownloadPreflight("bash", map[string]any{"command": command})
		if preflight == nil || preflight["status"] != "durable_download_preflight_required" ||
			preflight["executed"] != false || !strings.Contains(stringValue(preflight["recovery"]), "Range/If-Range") {
	...
	for _, command := range []string{
		...
		if preflight := agentRuntimeBashDownloadPreflight("bash", map[string]any{"command": command}); preflight != nil {
