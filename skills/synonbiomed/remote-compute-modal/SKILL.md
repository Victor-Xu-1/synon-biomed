...
description: Run GPU jobs on the user's own Modal account via host.compute.create('modal', ...). Covers the create→submit→wait_for_notification flow, the compute_provider kernel for env setup, image/volume resolution, and the two approval cards. Load once you've decided to dispatch to Modal.
license: Apache-2.0
tools:
  - repl
  - compute_provider
...
If `compute.create('modal', …)` returns `unknown provider 'modal'`,
...
5. Whatever is under `./out/` (plus the logs) is tarred and staged
   ...
   post-job timer — the **idle watchdog** — terminates it after ~15 min
   of inactivity. See "When the job fails".

The **container timeout** (`provider_params.timeout`) is how long
...
`host.compute.create('modal', provider_params={...})` is
...
prompts. The kernel rejects `gpu=` and any `timeout` above 30 min on
`Sandbox.create`; it never silently clamps either value. This is the
kernel's own probe budget, separate from the
...
*kernel*, never your sandboxes — the ≤30-min sandbox bound limits how long
...
job = c.submit_job(
    ...
    inputs=[{'src': 'target.fasta', 'dst': 'target.fasta'}],
...
`inputs=` stage **flat** into the workdir root. `dst` is a bare
...
`dst` can't. Omit it to default to `basename(src)`. Need a dir
...
`provider_params` uses **Modal's own kwarg names** — Modal's docs are
...
~15 min of inactivity (the sandbox's PID-1 **idle watchdog** — a kill,
...
watchdog terminates it after ~15 min of inactivity, and reading files
...
tags={"synon-biomed-job": "<job_id>"})` — the tag names the job
...
The Files tab lists `modal` alongside SSH hosts: at `/` it shows
...
`provider_params` field (the schema rejects `environment` /
...
# → {'provider': 'modal', 'environment': 'team-bio',
#    'app_name': 'synon-biomed-default', 'egress_mode': 'allowlist',
