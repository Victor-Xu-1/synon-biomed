...
from mcp_servers_common.criteria import blank, reject_blank
...
mcp = MCPServer("mcp-structures-interactions")
...
def pdb_search_structures(
    ...
    max_rows: int = 100,
    sort_by: Literal["relevance", "initial_release_date"] = "relevance",
    ...
    `sort_by` is `relevance` (default) or `initial_release_date`;
    release-date sorting also uses resolution ascending as a deterministic
    secondary criterion. `sort_direction` is `desc` or `asc`.
    ...
    1..1000, caps retrieval), the applied sort contract, and records
    [{pdb_id, score}] in that server-owned order. Identifiers only — chain to
    pdb_get_structures for metadata.
    """
    reject_blank(
        text=text,
        organism=organism,
...
def pdb_get_structures(pdb_ids: list[str]) -> dict[str, Any]:
    ...
    pdb_get_ligands), the primary citation (title, journal, year, authors,
    PubMed id, DOI), and coordinate_files.mmcif_url. The URL is the canonical
    RCSB download identity for a later governed download tool; this read-only
    metadata call never downloads coordinate files itself. Unknown ids come
    back as {"pdb_id", "error": "not_found"} — never silently dropped.
    """
    ...
    for record in records:
        ...
        if record.get("error") or len(pdb_id) != 4 or not pdb_id.isalnum():
            continue
        record["coordinate_files"] = {
...
@mcp.tool(annotations=READ_ONLY)
def pdb_select_latest_liganded_structure(
    text: str | None = None,
    organism: str | None = None,
    taxonomy_id: int | None = None,
    uniprot_accession: str | None = None,
    ...
    """Select the newest verified PDB entry with a substantive organic ligand.

    This composes one authoritative release-date-descending RCSB search with
    ordered entry and ligand hydration. It returns the first qualifying entry,
    its organic ligand records, and every newer rejected candidate with a
    machine-readable reason. The default chemistry screen requires at least
    four carbon atoms, formula weight >=120 Da, a SMILES, and excludes common
    crystallization solvents and buffers. Adjust those bounds only for a
    scientific question that explicitly includes fragment-sized ligands.

    At least one target filter is required. `max_candidates` is 1..25 and
    bounds the inspection. If no entry qualifies, selected=false; never
    describe an older manually sampled entry as the latest result.
    """
    reject_blank(
        text=text,
        organism=organism,
    ...
    if not 1 <= int(max_ligands) <= 25:
        raise ValueError("max_ligands must be 1..25")
    if min_formula_weight < 0:
        raise ValueError("min_formula_weight must be non-negative")
    if min_carbon_atoms < 1:
        raise ValueError("min_carbon_atoms must be at least 1")
    search = search_structures(
        _pdb_client(),
        text=text,
        organism=organism,
        taxonomy_id=taxonomy_id,
    ...
    result = select_latest_liganded_from_search(
        _pdb_client(),
        search,
        max_candidates=int(max_candidates),
        max_ligands=int(max_ligands),
        min_formula_weight=float(min_formula_weight),
        min_carbon_atoms=int(min_carbon_atoms),
        exclude_common_crystallization_components=exclude_common_crystallization_components,
    )
    result["search"] = {
        "total_count": search.get("total_count"),
        "n_retrieved": search.get("n_retrieved"),
