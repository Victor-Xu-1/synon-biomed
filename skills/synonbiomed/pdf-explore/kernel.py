...
PDF_DEFAULT_MODEL = None
"""None means the host uses the active third-party LLM profile."""
...
def pdf_pages(path, mode="auto", pages=None, dpi=100, cache=True):
    ...
    ``ImportError`` with the ``manage_packages`` repair recipe if none is
    available.
    """
    ...
    if pdfium is not None:
        ...
        if render:
            raise ImportError(
                ...
                "if that is rejected for the platform-managed default, create "
                "a domain environment with pypdfium2 and pillow and re-run there."
            )
        try:
            ...
            raise ImportError(
                ...
                "manage_packages(environment='python', mode='install', "
                "packages=['pypdfium2', 'pillow'], use_pip=True) and re-run "
                "(pillow is needed for mode='image'); if that is rejected for "
                "the platform-managed default, create a domain environment "
                "with pypdfium2 and pillow and re-run there."
            ) from e
...
no recall gain from either; the bool is unreliable on image input (LLM
...
def pdf_scan(path, query, top_k=5, mode="auto", model=None,
    ...
    the doc is ≤150 text-layer pages (one LLM call with all pages +
    ...
    ``max_concurrency=16``. ``model`` defaults to the active third-party LLM profile when not passed.
    ...
    # One llm call with ALL pages in context + a rank tool. The active LLM compares
