...
import type {
  ...
  PendingSkill,
  SkillInfo,
} from '@/renderer/pages/settings/SynonBiomedExpertsSettings/types';
...
const loadAvailableSkills = async (): Promise<SkillInfo[]> => {
  return loadAvailableSkillsWithSynonBiomed<SkillInfo>();
};
...
export const useAssistantEditor = ({
  ...
  const handleSave = async () => {
    try {
      if (!editName.trim()) {
        message.error(t('settings.assistantNameRequired'));
        return;
      }

      if (defaultModelMode === 'fixed' && !defaultModelValue.trim()) {
        message.error(t('settings.assistantDefaultModelRequired'));
        return;
      }

      if (defaultPermissionMode === 'fixed' && !defaultPermissionValue.trim()) {
        message.error(t('settings.assistantDefaultPermissionRequired'));
        return;
      }

      if (defaultThoughtLevelMode === 'fixed' && !defaultThoughtLevelValue.trim()) {
        message.error(t('settings.assistantDefaultThoughtLevelRequired'));
        return;
      }
      ...
      if (pendingSkills.length > 0) {
        ...
        try {
          await skillsToImport.reduce<Promise<void>>(async (previousImport, pendingSkill) => {
            await previousImport;
            await ipcBridge.fs.importSkills.invoke({ skill_path: pendingSkill.path });
          }, Promise.resolve());
        } catch (error) {
          console.error('Failed to import assistant skill:', error);
          message.error(getSkillImportErrorMessage(error, t));
          return;
        }

        if (skillsToImport.length > 0) {
          const skillsList = await loadAvailableSkills();
          setAvailableSkills(skillsList);
        }
      }
  ...
  const handleDeleteClick = () => {
    ...
    if (activeAssistant?.source !== 'user') {
      message.warning(t('settings.cannotDeleteBuiltin'));
      return;
    }
