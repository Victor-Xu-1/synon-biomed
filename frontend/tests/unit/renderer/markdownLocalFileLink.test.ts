...
describe('resolveLocalFileLinkPath', () => {
  it('recognizes Windows absolute paths emitted as root-relative markdown links', () => {
    expect(resolveLocalFileLinkPath('/C:/Users/Administrator/AppData/Roaming/SynonAI/report.xlsx')).toBe(
  ...
  it('recognizes file-like POSIX absolute paths outside common home and temp roots', () => {
    expect(resolveLocalFileLinkPath('/opt/synon-ai/outputs/report.xlsx')).toBe('/opt/synon-ai/outputs/report.xlsx');
  });

  it('recognizes line suffixes without confusing Windows drive letters', () => {
    const reference = resolveLocalFileLinkReference('C:/Users/Administrator/AppData/Roaming/SynonAI/logs/app.log:1421');

    expect(reference).toEqual({
      filePath: 'C:/Users/Administrator/AppData/Roaming/SynonAI/logs/app.log',
      rawReference: 'C:/Users/Administrator/AppData/Roaming/SynonAI/logs/app.log:1421',
      line: 1421,
    });
    expect(resolveLocalFileLinkPath('C:/Users/Administrator/AppData/Roaming/SynonAI/logs/app.log:1421')).toBe(
  ...
  it('recognizes line and column suffixes without including the line in the file path', () => {
    const reference = resolveLocalFileLinkReference(
      'C:/Users/Administrator/AppData/Roaming/SynonAI/logs/app.log:1421:7'
    );
    ...
    expect(resolveLocalFileLinkPath('C:/Users/Administrator/AppData/Roaming/SynonAI/logs/app.log:1421:7')).toBe(
      'C:/Users/Administrator/AppData/Roaming/SynonAI/logs/app.log'
    );
  });
  ...
  it('rejects unsupported hash line formats and remote hash links', () => {
    ...
    expect(resolveLocalFileLinkReference('https://synon-ai.com/docs#L10')).toBeNull();
  ...
  it('does not treat normal web links or app routes as local files', () => {
    expect(resolveLocalFileLinkPath('https://synon-ai.com/docs')).toBeNull();
    expect(resolveLocalFileLinkPath('/settings')).toBeNull();
  });

  it('formats local file paths as file URLs for browser link copying', () => {
    expect(toLocalFileHref('C:/Users/Administrator/AppData/Roaming/SynonAI/report.xlsx')).toBe(
      'file:///C:/Users/Administrator/AppData/Roaming/SynonAI/report.xlsx'
    );
