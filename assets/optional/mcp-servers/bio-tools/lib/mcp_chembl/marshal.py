...
def compound_search_response(records: list[dict], total: int | None,
                             offset: int = 0) -> dict:
    ...
    return {
        ...
        "next_offset": (offset + len(compounds)
                        if offset + len(compounds) < total else None),
        "compounds": compounds,
...
def drug_search_response(pairs: list[tuple[dict, dict]], total: int,
                         ...
                         total_indication_rows: int | None,
                         offset: int = 0,
                         candidate_count: int | None = None) -> dict:
    drugs = [drug_record(m, joined) for m, joined in pairs]
    scanned = len(drugs) if candidate_count is None else candidate_count
...
def bioactivity_response(raw_activities: list[dict], total: int | None,
                         offset: int = 0) -> dict:
    ...
    return {
        ...
        "offset": offset,
        "has_more": has_more,
        "next_offset": offset + len(activities) if has_more else None,
...
def mechanism_response(raw_mechanisms: list[dict], total: int | None,
                       offset: int = 0) -> dict:
    ...
    has_more = offset + len(mechanisms) < total
    return {
        "count": len(mechanisms),
        "total": total,
        "offset": offset,
        "has_more": has_more,
        "next_offset": offset + len(mechanisms) if has_more else None,
...
def target_search_response(raw_targets: list[dict], total: int | None,
                           offset: int = 0) -> dict:
    ...
    return {
        ...
        "next_offset": (offset + len(targets)
                        if offset + len(targets) < total else None),
        "targets": targets,
        # NEW (additive): explicit truncation flag.
