...
export const NavigationInterceptor = {
   ...
   */
  normalizeToolName(tool_name: string): string {
    if (!tool_name) return '';
   ...
   */
  isChromeDevToolsIdentifier(str: string): boolean {
    if (!str) return false;
   ...
   */
  isNavigationTool(data: NavigationToolData | string): boolean {
   ...
   */
  extractUrl(data: NavigationToolData): string | null {
    // 1. Direct url field
   ...
   */
  extractUrlFromObject(obj: Record<string, unknown>): string | null {
   ...
   */
  createPreviewMessage(url: string, conversation_id: string, title?: string): IResponseMessage {
    return {
      type: 'preview_open',
   ...
   */
  intercept(data: NavigationToolData, conversation_id: string): InterceptionResult {
