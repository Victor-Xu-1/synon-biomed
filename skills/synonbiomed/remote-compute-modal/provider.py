...
# in the llm-bioscience rebrand (operon_owner →
# llm-bioscience-install-id), then in the bioscience→science rename. Both
...
# new key.
OWNER_TAG = "synon-biomed-install-id"
LEGACY_OWNER_TAGS = ("llm-science-install-id", "llm-bioscience-install-id", "operon_owner")
SCIENTIFIC_JOB_TAG = "synonbiomed-job"
SCIENTIFIC_SUBMISSION_TAG = "synonbiomed-submission"
...
LEGACY_APP_NAMES = ("llm-science-default", "operon-modal", "egress-staging")
...
FALLBACK_APP_NAME = "synon-biomed-default"
KERNEL_TIMEOUT_CAP_S = 1800


def _plain_str(value: Any) -> str:
    """Return an exact built-in string before applying identity policy."""
    text = value if isinstance(value, str) else str(value)
    return text if type(text) is str else "".join((text,))
...
def _sanitize_repl_tags(tags: Any, self_id: str) -> Any:
    ...
    id snapshotted when the guard is installed — that is
    ...
    if not isinstance(tags, dict):
        if isinstance(tags, _abc.Mapping):
            ...
            raise ByocError("invalid_request", "tags must be a dict of str -> str")
    out = {}
...
def _repl_identity_tags() -> dict[str, str]:
    """Snapshot the host-stamped identity for every repl-created sandbox."""
    identity = {
        key: value
        for key, value in (
    ...
    return identity if OWNER_TAG in identity else {}
...
# never landed) leaks at most ~15 min of idle billing instead of the full
...
# container default that backstop is 12h+900s. 15 min is also the post-job
# harvest grace: the poller normally harvests within seconds of `.phase`
# appearing, so only a daemon that is down for >15 min loses outputs to
...
def _watchdog_cmd(work: str = WORK, idle_exit_s: float = IDLE_EXIT_S,
    ...
    happens after 30 idle polls = 900 s at the production constants."""
...
class ModalProvider:
    ...
    def import_and_patch(self) -> None:
        ...
        _app_cfg = os.environ.get("OPERON_BYOC_APP_NAME")
        if _app_cfg:
            self.set_app_name(_app_cfg)
    ...
    def _owned_app_key_combinations(self) -> list[tuple[Any, str]]:
        current = self._get_app()
        combos: list[tuple[Any, str]] = [
            (current, OWNER_TAG),
            *[(current, key) for key in LEGACY_OWNER_TAGS],
        ]
        for extra_name in (FALLBACK_APP_NAME, *LEGACY_APP_NAMES):
            if extra_name == self._app_name:
                continue
            extra = self._lookup_existing_app(extra_name)
            if extra is not None:
                combos.append((extra, OWNER_TAG))
                combos.extend((extra, key) for key in LEGACY_OWNER_TAGS)
        for prior_name in self._prior_app_names:
            if prior_name in (self._app_name, FALLBACK_APP_NAME, *LEGACY_APP_NAMES):
                continue
            prior = self._lookup_existing_app(prior_name)
            if prior is not None:
                combos.append((prior, OWNER_TAG))
                combos.extend((prior, key) for key in LEGACY_OWNER_TAGS)
        return combos
    ...
    def list_owned(self, install_id: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            seen: set[str] = set()
            for app, key in self._owned_app_key_combinations():
    ...
    def find_owned_submission(
            self, install_id: str, job_id: str,
            submission_id: str) -> list[dict[str, Any]]:
        """Return a bounded exact lookup for crash-safe submission adoption.

        Every SDK iterator is fully exhausted so a late duplicate cannot be
        hidden by pagination. Results are deduplicated by object id and only
        the first two are retained: 0 means create, 1 means adopt, and 2 means
        the durable identity is ambiguous and must fail closed.
        """
        m = self._modal
        try:
            out: list[dict[str, Any]] = []
            seen: set[str] = set()
            for app, owner_key in self._owned_app_key_combinations():
                query = {
                    owner_key: install_id,
                    SCIENTIFIC_JOB_TAG: job_id,
                    SCIENTIFIC_SUBMISSION_TAG: submission_id,
                }
                for sandbox in m.Sandbox.list(app_id=app.app_id, tags=query):
                    ...
                    if (
                        ...
                        or tags.get(owner_key) != install_id
                        or tags.get(SCIENTIFIC_JOB_TAG) != job_id
                        or tags.get(SCIENTIFIC_SUBMISSION_TAG) != submission_id
                    ):
                        raise ByocError(
                            "provider_degraded",
                            "owned submission tag readback did not match the exact query",
                        )
                    if sandbox_id in seen:
                        continue
                    seen.add(sandbox_id)
                    if len(out) < 2:
                        out.append({"sandbox_id": sandbox_id, "tags": tags})
            return out
        except ByocError:
            raise
        except Exception as e:  # noqa: BLE001
    ...
    def list_volumes(self) -> list[dict[str, Any]]:
        ...
        view for ``modal`` at path ``/``. ``created_at`` is epoch ms
    ...
    def _install_gpu_guard(self, m: Any) -> None:
        identity = _repl_identity_tags()
        self_id = identity.get(OWNER_TAG, "")
        ...
        def guarded(*a, **kw):
            if kw.get("gpu"):
                raise ByocError(
                    "invalid_request",
                    "gpu= is not available in the compute_provider kernel; "
                    "GPU and long-running work must use host.compute.create("
                    "'modal', provider_params=...) and submit_job(), which "
                    "applies the job approval and durable lifecycle authority.",
                )
            ...
            if t is not None:
                ...
                if timeout > KERNEL_TIMEOUT_CAP_S:
                    raise ByocError(
                        "invalid_request",
                        f"timeout={timeout} exceeds the compute_provider kernel's "
                        f"{KERNEL_TIMEOUT_CAP_S} s sandbox cap; use the durable "
                        "host.compute job surface for longer work.",
                    )
                kw["timeout"] = timeout
            if kw.get("tags") is not None:
                kw["tags"] = _sanitize_repl_tags(kw["tags"], self_id)
            current = kw.get("tags")
            if identity and (current is None or isinstance(current, dict)):
                merged = {**identity, **(current or {})}
                merged[OWNER_TAG] = identity[OWNER_TAG]
                kw["tags"] = merged
            ...
            # other defaults this guard injects (the gpu reject and timeout
            # bound) are constants; the egress policy is per-provider DB
            ...
            # boundary on this path remains the confined provider kernel,
            # the provider-kernel approval, and the remote workspace quota.
            return orig(*a, **kw)

        m.Sandbox.create = staticmethod(guarded)

        # Same soft-boundary tier as the gpu/timeout bound above: repl cells
        ...
        def guarded_set_tags(self_, tags, *a, **kw):
            ...
            if isinstance(merged, dict):
                ...
                # pre-rename sandboxes carry llm-bioscience-* identity
                # tags and must keep them for the reaper's dual-read.
                for k, v in cur.items():
                    if (k in LEGACY_OWNER_TAGS
                            or k.startswith("synon-biomed-")
                            or k.startswith("llm-science-")
                            or k.startswith("llm-bioscience-")):
                        merged[k] = v
                owned_here = OWNER_TAG in cur or any(
                    cur.get(key) == self_id for key in LEGACY_OWNER_TAGS
                )
                if not owned_here:
    ...
    def _map_err(self, e: Exception) -> ByocError:
        ...
        if "deployed apps" in low and "limit of" in low:
            return ByocError(
                "quota_exhausted",
                "The Modal workspace is at its deployed-apps cap. Reuse the "
                "configured application or ask the workspace owner to stop "
                f"an obsolete application. ({raw})",
            )
        if ("RateLimit" in s or "TOO_MANY" in s
                or "rate limit" in low or "too many layers" in low):
            return ByocError(
                "rate_limited",
                "Modal rate-limited this request. Back off for about 60 seconds, "
                "stagger fan-out, and batch repeated volume operations. "
                f"({raw})",
            )
