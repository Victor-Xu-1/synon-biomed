...
search.rcsb.org and data.rcsb.org), identifying User-Agent, up to THREE
retries on 429/5xx/transport errors with exponential backoff (tool budget:
every call must finish well inside the 60 s MCP transport limit - timeout
11 s, capped backoff). Four attempts plus 2/4/8 s backoff remain below the
60 s MCP transport budget even when every attempt times out.
"""
    ...
    def _request(self, method: str, url: str, json_body: dict | None = None) -> httpx.Response | None:
        ...
        for attempt in range(self.max_retries + 1):
            ...
            if resp.status_code in RETRY_STATUS and attempt < self.max_retries:
                ...
                base_wait = retry_after_seconds(retry_after, 2.0, cap=10.0)
