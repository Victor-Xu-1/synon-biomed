...
# Common annotation labels used by scientists and emitted by several model
# families are not the field identifiers accepted by UniProt REST. Keep this
# mapping deliberately small and one-to-one; unknown fields still reach the
# upstream validation boundary instead of being guessed.
_FIELD_ALIASES = {
...
def _normalize_fields(fields: Iterable[str]) -> list[str]:
    ...
    for value in fields:
        ...
        if not field:
            raise ValueError("UniProt field names must not be blank")
        canonical = _FIELD_ALIASES.get(field.casefold(), field)
