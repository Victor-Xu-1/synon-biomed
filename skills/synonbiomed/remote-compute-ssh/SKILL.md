...
Then `job = c.submit_job(...)` (see below). `inputs=[{src:'file', dst:
...
reads `./<dst>` from its own workdir. End the cell — `submit_job`
...
need the full result dict,
re-enter a `repl` cell and call `r = c.attach_job(job_id).result()` — a
non-blocking read of what the poller already harvested.
`open(r['output_files'][i])` reads any harvested file directly. Move any
additional regular file with the canonical `scp` tool; compute handles do not
expose a competing upload/download API.
...
`command` is a job script. For `scheduler: slurm`, the host hoists bounded
`#SBATCH ` lines from the top into the dispatch script. GPU/time/partition/
account are yours. Don't write `--array`/`--chdir`/`--wrap`; submit one job
per task. For `scheduler: none`, omit directives. Other scheduler families
fail before staging or allocation.
...
there as `./<dst>`. `dst` is a workspace-relative filename without traversal.
Only `./out/` plus stdout/stderr logs are harvested, so point the tool's
output directory at `./out` or copy final results there before exit. Anything
outside the job workdir is never auto-harvested; move it explicitly with
`scp` only when needed. End with `ls -lh out/` when output discovery is
uncertain so the log proves what existed before harvest.
...
`inputs` accepts a workspace-relative string, `{src, dst?}`, or the literal
`{{artifact:VERSION_ID}}` marker. Artifact bytes are streamed through the
task-owned artifact store; a foreign, missing, or pruned version fails before
allocation. `ssh://<target>/<absolute-path>` is accepted only for this same
target after server-side canonicalization proves it is a regular file under a
declared data or scratch root; it is symlinked into the workdir and never
downloaded/re-uploaded. `outputs` is a featured-view selector over files written under
`./out/`: a string is featured and `{glob, visibility:'hidden'}` retains a
diagnostic in `output_files` without placing it in `featured_files`. Logs are
always harvested. Use `scp` for sources outside those declared roots.
`exclude=['tmp/**']` removes matching output paths intentionally.
`transfer_limits={'max_file_mb':500,'max_total_mb':2000}` bounds extraction;
unselected or over-limit files are reported in `left_on_remote` and keep the
validated remote workdir alive. Chain one with its returned `ssh://` URI, or
call `c.close()` after deciding the retained files can be deleted.

```python
# repl tool — host.compute isn't attached in the `python` tool
c = host.compute.create('<cluster>')
job = c.submit_job(
...
ls -lh ./*.result ./*.json''',
    inputs=[
        {'src': 'in.dat', 'dst': 'in.dat'},               # workspace-relative (prepared in a `python` cell)
        {'src': '{{artifact:<id>}}', 'dst': 'ref.dat'},   # artifact marker — either form works
        # or chain a prior job's harvest: {'src': prev_featured[0], 'dst': 'prev.out'}
    ],
    outputs=[
        '*.result',                                                  # featured
...
If you need the fuller result dict (`output_files`, `remote_workdir`,
`stdout_tail`):

```python
# repl tool — non-blocking read of what the poller already harvested
r = c.attach_job(job_id).result()
# r → {state, exit_code, output_files, featured_files, remote_workdir, ...}
c.close()
```
...
that would exceed it raises `host.compute.ConcurrencyFull`; never retry it in
a loop. When this frame owns a live job, end the cell and park on
`wait_for_notification`. When sibling jobs hold every slot, hand control back
to the orchestrator because their notifications will not wake this frame.
...
jobs = [
    c.submit_job(
        ...
        inputs=[{"src": "input.fasta", "dst": "input.fasta"}],
