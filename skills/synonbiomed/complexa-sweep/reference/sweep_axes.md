...
| `success_rate` | `mean(passes_filter)` when the executed analyzer emitted that field; otherwise apply only thresholds resolved from the checked result-type config or explicit acceptance contract | Never insert generic cutoffs when the result schema or threshold authority is missing. |
