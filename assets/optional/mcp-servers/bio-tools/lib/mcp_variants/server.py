...
server defaults every gnomAD tool to ``gnomad_r4`` (currently backed by the
gnomAD v4.1.1 browser release) and pins the dataset in
every output.
"""
...
def get_variant(variant_id: str, dataset: str = DEFAULT_DATASET) -> dict:
    ...
    Args:
        ...
        dataset: gnomAD dataset pin. Default ``gnomad_r4`` (v4.1.1,
            exomes+genomes). Other values: ``gnomad_r4_non_ukb``,
