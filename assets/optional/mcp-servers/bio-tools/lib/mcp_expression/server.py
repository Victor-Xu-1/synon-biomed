"""mcp-expression: gene/tissue expression domain server (MCPServer, stdio).
...
Tools are thin marshalling wrappers over two fleet packages:
  ...
  * gtex-expression   — complete, count-verified GTEx Portal API v2 retrieval
                        ...
                        pinned explicitly on every call, default gtex_v10).
...
from mcp_servers_common.criteria import reject_blank
...
def gtex_tissue_sites(dataset_id: str = "gtex_v10") -> dict:
    """List all GTEx tissue sites with metadata for a pinned dataset release.

    Returns every tissue site (54 in gtex_v10) with tissueSiteDetailId, display
    ...
    dataset_id pins the GTEx release (default 'gtex_v10', the current Adult
    GTEx release, GENCODE v39/GRCh38; gtex_v8 uses GENCODE v26).
    """
...
def gtex_dataset_info() -> dict:
    ...
    """
    return _gtex("gtex_v10").dataset_info()
...
def gtex_sample_info(tissue_site_detail_id: str | None = None,
    ...
    """
    reject_blank(tissue_site_detail_id=tissue_site_detail_id,
...
def gtex_median_expression(gencode_ids: list[str],
    ...
    tissue_site_detail_ids accepts official ids or exact display labels from
    gtex_tissue_sites (for example `Breast_Mammary_Tissue` or
    `Breast - Mammary Tissue`). Omit it to get all tissues. Fully paged and
...
@mcp.tool(annotations=READ_ONLY)
def gtex_expression_summary(gene: str, dataset_id: str = "gtex_v10") -> dict:
...
def gtex_eqtl_genes(tissue_site_detail_id: str,
    ...
    `total` is the API-verified count for the selected release and tissue.
