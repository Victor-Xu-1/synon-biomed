...
If the primary source is unavailable, do not invent an internal host, mirror,
or rsync path. Load `capability-acquisition` and use only a currently configured
and user-approved source whose license, immutable weight identity, checksum,
destination, and data boundary are verified. Preserve the transfer receipt and
validate the selected config against the resulting exact checkpoint paths.

An alternate layout is not interchangeable with the CLI's flat `./ckpts/`
layout. Update the selected config only when the verified source documents its
layout, and never describe an unavailable private mirror as a generic fallback.
