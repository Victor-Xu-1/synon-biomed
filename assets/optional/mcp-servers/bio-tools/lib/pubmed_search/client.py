...
class PubMedSearch:
    ...
    def _request(self, url: str, params: dict[str, Any]) -> requests.Response:
        """Send one endpoint-correct request with pacing and bounded retries.

        NCBI E-utilities accept POST bodies, which keeps api_key out of URLs.
        The separate PMC ID Converter ignores POST bodies and requires query
        parameters, so that endpoint uses GET without forwarding api_key.
        """
        ...
        for attempt in range(self.max_retries + 1):
            ...
            try:
                identified = self._ident(params)
                if url == IDCONV_URL:
                    # The ID Converter is not an E-utility and returns HTTP 400
                    # when ``ids`` is sent in a POST body. It also does not use
                    # NCBI API keys, so never expose a shared key in its URL.
                    identified.pop("api_key", None)
                    resp = self.session.get(
                        url, params=identified, timeout=self.timeout
                    )
                else:
                    resp = self.session.post(
                        url, data=identified, timeout=self.timeout
                    )
