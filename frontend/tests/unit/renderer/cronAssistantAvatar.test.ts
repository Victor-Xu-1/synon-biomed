...
describe('resolveAssistantAvatar', () => {
  ...
  it('does not derive avatar routes from local absolute assistant asset paths', () => {
    expect(resolveAssistantAvatar('/Users/demo/.synon-ai/assistant-avatars/custom-1.jpg')).toEqual({
      kind: 'fallback',
    });
  });
  ...
  it('does not expose file urls as image sources without an assistant id', () => {
    expect(resolveAssistantAvatar('file:///Users/demo/.synon-ai/assistant-avatars/custom-1.jpg')).toEqual({
      kind: 'fallback',
    });
  });
