...
Load `complexa-setup` and use its real CLI/compute readiness contract. Confirm
the exact pipeline and target with `complexa download --status`, `complexa
validate target`, and `complexa validate design`; inspect local or remote
capacity with `list_compute`. No shared preflight helper or preflight JSON is
bundled here.

A sweep multiplies the cost of one validated run by the Cartesian-product size.
Use a measured bounded pilot for the selected pipeline when available; otherwise
use a current official range and label it as an estimate.
...
When the resulting cost, quota use, latency, or scope is a material user-owned
trade-off, call `ask_user` once with two to four viable options such as the full
sweep, a reduced scientifically balanced design, or cancellation. Recommend one
using the measured capacity and explain each option's coverage, limitation,
resource/cost requirement, and expected output. Do not use a fixed GPU-hour
threshold as a substitute for the user's approved budget and provider quota.
...
Load `complexa-design` and reuse its selected pipeline/target decision rather
than reconstructing that choice from prompt phrases. Capture:
...
This is the only flag-supported entrypoint. The launcher: (1) generates N config pairs via `generate_inference_configs.py`, (2) names the run `{base}-search-{sweeper_basename}-{target}`, (3) rsyncs to the cluster (or runs in place with `--on-cluster`), (4) submits a SLURM array, one task per config. For cluster mechanics, load `complexa-slurm` and use its bundled script through the task-scoped Skill directory.
...
For each config, parse the analyze CSV (one row per generated binder). Standard
columns used for ranking include `i_pae`, `i_plddt`, `sc_rmsd`, `binder_seq`,
and `passes_filter`. Read exact names before calculation. If `passes_filter` is
missing, use only thresholds explicitly resolved from the checked result-type
config or the user's declared acceptance contract; otherwise leave success
unresolved rather than inventing generic cutoffs.
...
Preserve the exact sweeper file and hash, generated config list, pipeline and
target, overrides, immutable code/weight identity, environment generation,
provider/job receipts, per-config result paths, failed config IDs, summary CSV,
and Pareto selection. Publish the editable summary and requested scientific
outputs through `save_artifacts`. Keep raw logs and internal receipts out of the
default user file panel. Do not call an unbundled manifest helper or invent a
manifest that the executed route did not produce.
...
Use the selected backend's checked documentation, the live provider snapshot,
and a representative pilot for per-run memory and duration. Load
`complexa-slurm` for partition and array limits; do not rely on a missing shared
hardware file.
