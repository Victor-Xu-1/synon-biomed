...
``datasetId`` is an explicit argument everywhere (default ``gtex_v10``, the
current Adult GTEx release) rather than relying on an unpinned API default.
"""
...
class GtexExpression:
    ...
    def __init__(self, client: GtexClient | None = None,
        ...
        self._tissue_aliases: dict[str, set[str]] | None = None

    @staticmethod
    def _tissue_alias_key(value: str) -> str:
        return " ".join(str(value).strip().casefold().split())

    def _resolve_tissue_site_detail_ids(self, requested: list[str]) -> list[str]:
        if self._tissue_aliases is None:
            aliases: dict[str, set[str]] = {}
            for tissue in self.tissue_sites()["tissue_sites"]:
                tissue_id = str(tissue["tissueSiteDetailId"])
                for value in (
                    tissue_id,
                    tissue.get("tissueSiteDetail"),
                    tissue.get("tissueSiteDetailAbbr"),
                ):
                    if value is None or not str(value).strip():
                        continue
                    aliases.setdefault(self._tissue_alias_key(str(value)), set()).add(tissue_id)
            self._tissue_aliases = aliases

        resolved: list[str] = []
        seen: set[str] = set()
        for value in requested:
            label = str(value).strip()
            matches = self._tissue_aliases.get(self._tissue_alias_key(label), set())
            if not matches:
                raise ValueError(
                    f"unknown GTEx tissue {label!r} for {self.dataset_id}; "
                    "call gtex_tissue_sites and use tissueSiteDetailId or its exact display label"
                )
            if len(matches) != 1:
                raise ValueError(
                    f"ambiguous GTEx tissue {label!r} for {self.dataset_id}: {sorted(matches)}; "
                    "call gtex_tissue_sites and use tissueSiteDetailId"
                )
            tissue_id = next(iter(matches))
            if tissue_id not in seen:
                seen.add(tissue_id)
                resolved.append(tissue_id)
        return resolved
    ...
    def sample_info(self, tissue_site_detail_id: str | None = None,
        ...
        if tissue_site_detail_id:
            params["tissueSiteDetailId"] = self._resolve_tissue_site_detail_ids(
                [tissue_site_detail_id])[0]
        if data_type:
    ...
    def top_expressed(self, tissue_site_detail_id: str, n: int = 50,
        ...
        tissue_site_detail_id = self._resolve_tissue_site_detail_ids(
            [tissue_site_detail_id])[0]
