...
export function useWorkspaceDragImport({
  ...
  const resolveDroppedItems = useCallback(async (items: DroppedItem[]): Promise<DroppedItem[]> => {
    const resolvedItems = await Promise.all(
      items.map(async (item): Promise<DroppedItem> => {
        try {
          ...
          return { path: item.path, name: itemName, kind };
        } catch (error) {
          ...
          return { path: item.path, name: fallbackName, kind: 'file' };
        }
      })
    );

    return Array.from(new Map(resolvedItems.map((item) => [item.path, item])).values());
  }, []);
  ...
  const handleDrop = useCallback(
    async (event: DragEvent) => {
      ...
      if (targets.length === 0) {
        messageApi.warning(t('conversation.workspace.dragNoFiles'));
        return;
      }
