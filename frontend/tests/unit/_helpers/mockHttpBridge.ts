 ...
 *
 * SynonAI N3 mock HTTP/WS bridge helper.
 *
 * Public API frozen in docs/backend-migration/plans/2026-05-08-n3-test-rewrite-adapter-common.md §2.1.
 * Contributors: do not change the exported signatures. If a new capability is
...
export interface MockHttpBridge {
  ...
  // WS event emission
  /**
   * Dispatch payload to listeners subscribed via wsEmitter(kind, type).on(cb).
   * Synchronous dispatch: all listeners receive event before call stack unwinds.
   */
  emit(kind: 'durable' | 'direct' | 'control', eventType: string, payload: unknown): void;
  ...
  asModule(): {
    ...
    bindRealtimeRuntime: typeof import('@/common/adapter/httpBridge').bindRealtimeRuntime;
    realtimeReconnectedEmitter: typeof import('@/common/adapter/httpBridge').realtimeReconnectedEmitter;
    unavailableEmitter: typeof import('@/common/adapter/httpBridge').unavailableEmitter;
...
class MockHttpBridgeImpl implements MockHttpBridge {
  ...
  emit(kind: 'durable' | 'direct' | 'control', eventType: string, payload: unknown): void {
    const listeners = this.wsListeners.get(`${kind}:${eventType}`);
    if (!listeners) return;
    ...
    for (const cb of listeners) {
      try {
        ...
        console.warn('[mockHttpBridge] realtime listener failed', `${kind}:${eventType}`);
      }
    }
  }
  ...
  get wsListenerCount(): number {
    ...
    return total + this.reconnectListeners.size;
  }
  ...
  asModule(): {
    ...
    return {
      ...
      wsEmitter: <Params = undefined>(
        kind: 'durable' | 'direct' | 'control',
        eventType: string
      ): EmitterLike<Params> => {
        return {
          on: (callback: (params: Params) => void) => {
            ...
            if (!this.wsListeners.has(key)) {
              this.wsListeners.set(key, new Set());
            }
            const cb = callback as (payload: unknown) => void;
            this.wsListeners.get(key)!.add(cb);
      ...
      wsMappedEmitter: <Params = undefined>(
        ...
        const inner = this.asModule().wsEmitter(kind, eventType);
        return {
          on: (callback: (params: Params) => void) => {
      ...
      bindRealtimeRuntime: vi.fn(() => () => {}) as typeof import('@/common/adapter/httpBridge').bindRealtimeRuntime,
      ...
      unavailableEmitter: vi.fn(() => ({
        on: () => () => {},
      })) as typeof import('@/common/adapter/httpBridge').unavailableEmitter,
