...
def arxiv_search(query: str | None = None, category: str | None = None,
                 ...
                 start: int = 0, max_results: int = 50,
    ...
    Args:
        ...
        max_results: initial discovery page size (default 50, max 100 per
            call). Continue from ``start + n_records_returned`` while the
            upstream total indicates more results are needed.
    ...
    """
    query = none_if_blank(query)
    reject_blank(category=category)
