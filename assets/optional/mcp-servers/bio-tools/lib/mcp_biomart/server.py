...
``core/mcp-sources/biomart/biomart_mcp.py`` — MCPServer + pybiomart).
...
BIOMART_MARTS_FALLBACK = "\n".join([
    "name,display_name,description",
    "ENSEMBL_MART_ENSEMBL,Ensembl Genes,Gene annotation from Ensembl",
    "ENSEMBL_MART_MOUSE,Mouse strains,Mouse strains and variation data",
    "ENSEMBL_MART_ONTOLOGY,Ontology,Ontology and phenotype annotation",
    "ENSEMBL_MART_GENOMIC,Genomic features,Genomic feature annotation",
    "ENSEMBL_MART_SNP,Ensembl Variation,Variant annotation from Ensembl",
    "ENSEMBL_MART_FUNCGEN,Ensembl Regulation,Functional genomics and regulation",
])
...
def list_marts(args: dict) -> str:
    try:
        ...
        return (
            f"{BIOMART_MARTS_FALLBACK}\n"
            f"# fallback_used=true; upstream_error={str(exc).replace(chr(10), ' ')}"
        )
...
def get_data(args: dict) -> str:
    try:
        ...
        bad = sorted(
            ...
            if blank(value) or (isinstance(value, (list, tuple)) and (
                not value or any(not str(item).strip() for item in value)
            ))
        )
        if bad:
            raise ValueError(
                f"filter(s) {', '.join(bad)} have empty values — an empty "
                "filter matches nothing; omit the filter instead"
            )
        _, filter_types = _configuration(dataset)
        xml = marshal.build_query_xml(
            dataset, args["attributes"], filters,
