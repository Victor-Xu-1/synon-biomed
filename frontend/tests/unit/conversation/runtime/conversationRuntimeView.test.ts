...
describe('conversationRuntimeViewStore', () => {
  ...
  it('keeps a completed capsule finalizing until the matching terminal answer is rendered', () => {
    ...
    const terminal = hydrateSucceededConversationRuntimeView(running, conversation_id, runtime({})).view;
  ...
  it('releases the terminal projection only through the rendered-message acknowledgement', () => {
    resetConversationRuntimeViewStoreForTest();
    hydrateSucceeded(
      conversation_id,
      runtime({
    ...
    expect(getConversationRuntimeViewSnapshot(conversation_id).terminalProjectionPending).toBe(true);

    markTerminalProjectionReady(conversation_id);
  ...
  it('hydrates a user-paused task as sendable and starts a new continuation cleanly', () => {
    const paused = hydrateSucceededConversationRuntimeView(
      undefined,
      conversation_id,
      runtime({
        state: 'paused',
    ...
    expect(localSendStartedConversationRuntimeView(paused, conversation_id).view).toMatchObject({
      state: 'starting',
      isProcessing: true,
      canSendMessage: false,
  ...
  it('hydrates plan approval as sendable and starts the submitted decision cleanly', () => {
    const waitingApproval = hydrateSucceededConversationRuntimeView(
      undefined,
      conversation_id,
      runtime({
    ...
    expect(localSendStartedConversationRuntimeView(waitingApproval, conversation_id).view).toMatchObject({
      state: 'starting',
      isProcessing: true,
      canSendMessage: false,
      localSubmitting: true,
