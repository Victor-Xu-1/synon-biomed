...
import { uploadSynonBiomedArtifact } from '@/renderer/services/synonBiomedWorkspaceApi';
...
export function useWorkspacePaste(options: UseWorkspacePasteOptions) {
  ...
  const copyFilesIntoWorkspace = useCallback(
    async (selectedFiles: string[]) => {
      ...
      if (failedFiles.length > 0) {
        messageApi.warning(t('common.fileAttach.copyPartial'));
      }
    },
    ...
    if (!fileInputRef.current) {
      ...
      input.addEventListener('change', async () => {
        ...
        try {
          const uploads = Array.from(fileList).map(async (file) => {
            ...
            try {
              if (workspace.startsWith('synonbiomed://')) {
                await uploadSynonBiomedArtifact(file, conversation_id, tracker.onProgress, {
              ...
              if (!(error instanceof Error && error.message === UPLOAD_ABORTED_ERROR)) {
                messageApi.error(t('common.fileAttach.failed'));
              }
              return false;
            } finally {
              tracker.finish();
          ...
          if (successCount > 0) {
            messageApi.success(t('common.fileAttach.uploadSuccess'));
  ...
  }, [conversation_id, handleSelectHostFiles, messageApi, refreshWorkspace, t, workspace]);
    ...
    async (filesMeta: { name: string; path: string }[]) => {
      ...
      if (skipConfirm) {
        try {
          ...
          if (copiedFiles.length > 0) {
            messageApi.success(t('common.fileAttach.pasteSuccess'));
            setTimeout(() => refreshWorkspace(), 300);
          }
          ...
          messageApi.error(t('common.fileAttach.pasteFailed'));
        } finally {
