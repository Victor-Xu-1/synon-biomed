...
import AcpModelSelector from '@/renderer/components/synonBiomed/runtime/SynonBiomedModelSelector';
...
vi.mock('@icon-park/react', () => ({
  ...
  Attention: () => <span aria-hidden='true'>attention</span>,
...
vi.mock('react-i18next', () => ({
  useTranslation: () => ({
    t: (key: string, options?: { defaultValue?: string }) => {
      ...
      if (key === 'conversation.welcome.modelListLoading') return 'Loading models';
      if (key === 'conversation.welcome.modelListUnavailable') return 'Model list unavailable';
      if (key === 'conversation.welcome.modelListUnavailableHint') return 'The live model list failed to load.';
      if (key === 'conversation.welcome.modelListAuthError') return 'Model authentication failed';
      if (key === 'conversation.welcome.modelListAuthErrorHint') return 'Check Models settings and retry.';
      if (key === 'conversation.welcome.retryModelList') return 'Retry loading model list';
...
describe('AcpModelSelector runtime options', () => {
  ...
  it('renders a distinct loading state before model capability is known', () => {
    useAcpModelInfoMock.mockReturnValue(makeResult({ model_info: null, canSwitch: false, isModelListLoading: true }));

    render(<AcpModelSelector conversation_id='conversation-1' backend='codex' />);

    expect(screen.getByTestId('model-picker-loading')).toHaveTextContent('Loading models');
    expect(screen.queryByText('Use CLI model')).not.toBeInTheDocument();
  });

  it('keeps a visible degraded state and retries a failed live model load', async () => {
    const retryModelList = vi.fn().mockResolvedValue(undefined);
    useAcpModelInfoMock.mockReturnValue(
      makeResult({
        model_info: null,
        canSwitch: false,
        modelListError: new Error('provider returned api_key=secret-value'),
        retryModelList,
      })
    );
    ...
    const retry = screen.getByRole('button', { name: 'Retry loading model list' });
    expect(screen.getByTestId('model-picker-degraded-hint')).toHaveTextContent('Model list unavailable');
    expect(screen.queryByText(/secret-value/)).not.toBeInTheDocument();
    fireEvent.click(retry);
    await waitFor(() => expect(retryModelList).toHaveBeenCalledOnce());
  });

  it('distinguishes provider authentication failures without exposing the backend response', () => {
    useAcpModelInfoMock.mockReturnValue(
      makeResult({
        model_info: null,
        canSwitch: false,
        modelListError: new BackendHttpError({
          method: 'POST',
          path: '/api/conversations/conv-1/runtime',
          status: 401,
          body: { error: 'invalid key sk-test-secret' },
        }),
      })
    );
    ...
    expect(screen.getByTestId('model-picker-degraded-hint')).toHaveTextContent('Model authentication failed');
    expect(screen.queryByText(/sk-test-secret/)).not.toBeInTheDocument();
  });

  it('renders the thought level group before the ungrouped model options', () => {
    render(<AcpModelSelector conversation_id='conversation-1' backend='codex' />);

    expect(screen.getAllByRole('group').map((group) => group.getAttribute('aria-label'))).toEqual(['Thinking Level']);
    expect(screen.queryByRole('group', { name: 'Model' })).not.toBeInTheDocument();
    expect(
      within(screen.getByTestId('dropdown-menu')).getByText('GPT-5.2').closest('[role="menuitem"]')
    ).toBeInTheDocument();
  ...
  it('marks the current model with the same leading check indicator as thought level options', () => {
    ...
    const currentModelItem = screen.getByText('GPT-5.2').closest('[role="menuitem"]');
    const otherModelItem = screen.getByText('GPT-5.2 Mini').closest('[role="menuitem"]');
  ...
  it('shows model descriptions in option tooltips', () => {
    ...
    expect(screen.getByText('Opus').closest('[data-tooltip-content]')).toHaveAttribute(
      'data-tooltip-content',
