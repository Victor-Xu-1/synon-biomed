...
const TITLE_I18N_KEYS: Record<DocType, string> = {
...
const OfficeWatchViewer: React.FC<OfficeWatchViewerProps> = ({
  ...
  useEffect(() => {
    ...
    const start = async () => {
      ...
      setError(null);
      try {
        const result = await bridge.start.invoke({
          ...(file_path ? { file_path } : {}),
        ...
        if (!cancelled) {
          ...
          console.warn('[OfficeWatchViewer] Preview startup failed');
    ...
    return () => {
      cancelled = true;
      unsubStatus();
      if (file_pathRef.current || artifactId) {
        bridge.stop
          .invoke({
            ...(file_pathRef.current ? { file_path: file_pathRef.current } : {}),
            ...(artifactId ? { artifact_id: artifactId } : {}),
            ...(versionId ? { version_id: versionId } : {}),
          })
          .catch(() => {
            console.warn('[OfficeWatchViewer] Preview cleanup failed');
          });
      }
    };
  }, [artifactId, docType, file_path, retryKey, t, versionId, workspace]);
  ...
  if (isElectronDesktop()) {
    return <WebviewHost url={watchUrl} title={t(TITLE_I18N_KEYS[docType])} className='bg-1' />;
  }
  return <iframe src={watchUrl} className='w-full h-full border-0 bg-1' title={t(TITLE_I18N_KEYS[docType])} />;
};
