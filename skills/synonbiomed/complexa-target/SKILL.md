...
Use ask_user to collect — do not guess these. Required fields differ for protein vs ligand.
...
Then preserve the validated target definition with `save_artifacts` only when it
is a requested or useful user deliverable. Keep internal config paths and setup
receipts out of the default user file panel. The saved definition lets an
authorized future run compare or recreate the same target without guessing.
...
For downstream design or evaluation, defer to `complexa-design` and
`complexa-setup`; they derive resource requirements from the checked config,
live machine or provider, and actual backend instead of a missing shared table.
