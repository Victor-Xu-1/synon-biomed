...
describe('thinkTagFilter', () => {
  describe('stripThinkTags', () => {
    ...
    it('preserves public progress before a provider unused-thinking sentinel', () => {
      const input = '已确认最新结构，下一步获取配体信息。</think_never_used_51bce0c785ca2f68081bfa7d91973934>\n';
      expect(stripThinkTags(input)).toBe('已确认最新结构，下一步获取配体信息。\n');
    });

    it('preserves public text on both sides of a provider unused-thinking sentinel', () => {
      const input = '分子生成完成。</think_never_used_51bce0c785ca2f68081bfa7d91973934>接下来进行分子对接。';
      expect(stripThinkTags(input)).toBe('分子生成完成。接下来进行分子对接。');
    });

    it('removes complete provider-suffixed think blocks', () => {
      const input = 'Before<think_private_abc123>hidden</think_private_abc123>After';
      expect(stripThinkTags(input)).toBe('BeforeAfter');
    });
  ...
  describe('hasThinkTags', () => {
    ...
    it('detects provider-suffixed think tags', () => {
      expect(hasThinkTags('hidden</think_never_used_deadbeef>answer')).toBe(true);
    });
