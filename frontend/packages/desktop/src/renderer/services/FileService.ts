...
export async function uploadFileViaHttp(
  ...
  return new Promise<string>((resolve, reject) => {
    ...
    const endpoint = `${getBaseUrl()}/api/fs/upload`;
...
/**
 * Unrestricted file selector used by the message composer.
 *
 * The attachment pipeline accepts arbitrary files (including audio, video,
 * scientific formats, and user-defined extensions). `*` is kept as an
 * explicit sentinel so callers do not accidentally create a partial allowlist
 * when a new file type is added.
 */
...
import { SYNON_AI_TIMESTAMP_REGEX } from '@/common/config/constants';

// 清理SynonAI时间戳后缀，返回原始文件名
export function cleanSynonAITimestamp(file_name: string): string {
  return file_name.replace(SYNON_AI_TIMESTAMP_REGEX, '$1');
}

// 从文件路径获取清理后的文件名（用于UI显示）
export function getCleanFileName(file_path: string): string {
  const file_name = file_path.split(/[\\/]/).pop() || '';
  return cleanSynonAITimestamp(file_name);
}
...
class FileServiceClass {
  ...
  async processDroppedFiles(
    ...
    await Array.from(files).reduce<Promise<void>>(async (previousUpload, file) => {
