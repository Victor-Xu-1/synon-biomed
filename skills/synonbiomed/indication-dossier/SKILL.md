...
| Web | `web_search`, `web_fetch` — FDA guidance, treatment guidelines (NCCN, AASLD, specialty societies), CDC/WHO epidemiology data |
| Documents | `web_fetch` for remote PDFs; `read_file` for local PDFs |
| Subagents | `Agent` for parallel evidence gathering |

If a listed MCP isn't connected, say so and fall back to `web_search` against
the underlying public source (clinicaltrials.gov, pubmed.ncbi.nlm.nih.gov).

Use the cataloged read-only regulatory routes from `repl`; workflow labels are
not MCP method names:
...
labels = host.mcp(
    ...
    sections=["clinical_pharmacology", "pharmacokinetics",
...
`search_drug_labels` returns one object with `search`, `total`, `n_returned`,
`truncated`, and `records[]`. `search_trials` returns one structured search
object; preserve each returned NCT identifier and use
`host.mcp("clinical-trials", "get_trial_details", nct_id=...)` for a selected
trial. Do not invent `drug-regulatory-fda` as a Skill or MCP method.
