...
name: scvi-tools
description: >
  Advanced probabilistic single-cell RNA-seq with scVI or scANVI. Select only
  when verified raw integer UMI counts are available and the scientific task
  explicitly requires a probabilistic latent model, semi-supervised reference
  mapping, or Bayesian differential expression. Do not use for ordinary QC,
  normalization, Harmony-style batch correction, clustering, marker genes,
  population-composition analysis, or response signatures; use
  single-cell-rna-analysis for those workflows. This runtime requires a
  suitable accelerator and a complete prebuilt environment; never install
  scvi-tools or PyTorch incrementally during a task.
license: Apache-2.0
required-capabilities: [gpu]
...
class probabilities (one column per cell type). Keep `.max(axis=1)` beside
the labels and treat cells below an explicit threshold as low-confidence.

### Query-to-reference mapping (scArches)

Map a query dataset onto an already-trained reference instead of retraining:
...
The query needs the reference batch column in `.obs`. Build references meant
for later mapping with `encode_covariates=True` on the `SCVI` constructor so
the encoder is conditioned on batch/covariates; `load_query_data` can run
without it, but mapping quality suffers.
...
`fdr_target` selects the posterior expected-FDR level used for the
`is_de_fdr_*` column (default 0.05). `delta=None` replaces the fixed
log-fold-change threshold with one estimated from the data.
...
`compute_details({provider: 'modal', mode: 'read'})` for the current
image ref, then:
...
job = c.submit_job(
    ...
    inputs=[
        {"src": "dataset.h5ad", "dst": "dataset.h5ad"},
        {"src": "pipeline.py",  "dst": "pipeline.py"},
    ],
...
| Batches stay separated on the `X_scVI` UMAP (under-correction) | Raise `n_top_genes`, train longer, and verify that `batch_key` is the technical batch. |
| Known-distinct cell types merge (over-correction) | Use scANVI with the labels you do have; its label-aware embedding helps preserve known classes. |
