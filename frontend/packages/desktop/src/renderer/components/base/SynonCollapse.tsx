 ...
 */
export interface SynonCollapseProps {
 ...
 */
export interface SynonCollapseItemProps {
 ...
 */
const SynonCollapseItem: React.FC<SynonCollapseItemProps> = ({ children }) => <>{children}</>;
SynonCollapseItem.displayName = 'SynonCollapseItem';
 ...
 * ```tsx
 * // 基本用法 / Basic usage
 * <SynonCollapse defaultActiveKey={['1']}>
 *   <SynonCollapse.Item name="1" header="面板1">
 *     内容1
 *   </SynonCollapse.Item>
 *   <SynonCollapse.Item name="2" header="面板2">
 *     内容2
 *   </SynonCollapse.Item>
 * </SynonCollapse>
 *
 * // 手风琴模式 / Accordion mode
 * <SynonCollapse accordion defaultActiveKey="1">
 *   <SynonCollapse.Item name="1" header="面板1">内容1</SynonCollapse.Item>
 *   <SynonCollapse.Item name="2" header="面板2">内容2</SynonCollapse.Item>
 * </SynonCollapse>
 *
 * // 自定义图标 / Custom icon
 * <SynonCollapse
 ...
 * ```
 */
const SynonCollapseComponent: React.FC<SynonCollapseProps> & { Item: typeof SynonCollapseItem } = ({
  children,
  className,
  defaultActiveKey,
  activeKey,
  ...
  const items = useMemo(() => {
    return React.Children.toArray(children).filter((child): child is React.ReactElement<SynonCollapseItemProps> => {
      return React.isValidElement(child) && child.type === SynonCollapseItem;
    });
  }, [children]);
...
SynonCollapseComponent.Item = SynonCollapseItem;

export default SynonCollapseComponent;
