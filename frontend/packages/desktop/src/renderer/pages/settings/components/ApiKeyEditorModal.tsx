...
const ApiKeyEditorModal: React.FC<ApiKeyEditorModalProps> = ({ visible, api_keys, onClose, onSave, onTestKey }) => {
  ...
  const testAllKeys = useCallback(async () => {
    ...
    await keysToTest.reduce(
      (previousTest, key) => previousTest.then(() => executeKeyTest(key.id, key.value.trim())),
      Promise.resolve()
    );
  }, [keys, executeKeyTest]);
      ...
      <div className='flex flex-col gap-12px'>
        ...
        <div className='flex items-center justify-between pt-12px border-t border-line-2'>
          ...
          <div className='flex items-center gap-8px'>
            ...
            <Button className='flex' type='outline' size='small' onClick={addKey} style={{ minWidth: 70 }}>
              {t('common.add')}
            </Button>
          </div>
        </div>
