...
async function fetchJson<T>(method: string, path: string, body?: unknown): Promise<T> {
  ...
  const timeoutId = setTimeout(() => controller.abort('config_request_timeout'), CONFIG_REQUEST_TIMEOUT_MS);
  try {
    ...
    if (!response.ok) {
      throw new Error(`ConfigService ${method} ${path} failed (${response.status}): ${responseText}`);
    }
    ...
    if (controller.signal.aborted) throw new Error('config_request_timeout', { cause: error });
    throw error;
  } finally {
...
class ConfigServiceImpl {
  ...
  initialize(): Promise<void> {
    ...
    this.initPromise = (async () => {
      const data = await fetchJson<Record<string, unknown>>('GET', '/api/settings/client?scope=ui-preferences');
      this.cache.clear();
      if (data) {
      ...
      if (!this.cache.has('theme.activeId')) {
        ...
        // Browser WebUI bootstrap runs before local authentication. Keep the
        // migrated defaults in memory there; authenticated user changes still
        // persist through set()/setBatch().
        if (!isWebUiBrowserRuntime()) {
