"""mcp-genes-ontologies server — MCPServer tools over the bio-tools fleet.
...
mcp = MCPServer("mcp-genes-ontologies")
...
def get_go_annotations(
    ...
    """
    if blank(uniprot_accession):
        raise ValueError("uniprot_accession must be non-empty")
...
def get_uniprot_entries(
    ...
    - `fields` given: token-lean tabular retrieval of just those UniProt fields
      ...
      "length", "sequence"]); `format` is ignored. Common annotation aliases
      `function`, `subcellular_location`, and `domain` are deterministically
      mapped to UniProt REST fields `cc_function`,
      `cc_subcellular_location`, and `ft_domain`.
    ...
    if fields:
        ...
        for ln in lines:
            ...
            if cols == header:  # repeated header from a subsequent batch
                continue
            if len(cols) != len(fields) or len(header) != len(fields):
                raise RuntimeError(
                    "UniProt fields response does not match the requested field contract"
                )
            records.append(dict(zip(fields, cols)))
        return {
            "accessions": accessions,
            "fields": fields,
            "response_headers": header or [],
