...
Auto-generated by Synon Biomed instrument-data-to-allotrope skill
Generated: {timestamp}
Vendor: {vendor}
...
Requirements:
    Provision allotropy, pandas, and openpyxl in an isolated runtime.
...
try:
    ...
    print("Warning: allotropy is unavailable in the current runtime")
...
def convert_to_asm(filepath: str) -> Optional[Dict[str, Any]]:
    ...
    if not ALLOTROPY_AVAILABLE:
        raise ImportError("allotropy library required in the current runtime")
 ...
 "cells": [
  ...
  {{
   ...
   "source": [
    ...
    "# Provision allotropy, pandas, and openpyxl before running this notebook"
   ]
  }},
  {{
   "cell_type": "code",
