...
describe('useAcpModelInfo', () => {
  ...
  it('surfaces an initial model-list failure and recovers through an explicit retry', async () => {
    ensureRuntimeInvokeMock.mockRejectedValueOnce(new Error('temporary provider failure'));
    ...
    await waitFor(() => {
      expect(result.current.modelListError).toEqual(expect.any(Error));
    });
    expect(result.current.model_info).toBeNull();
    expect(result.current.isModelListLoading).toBe(false);
    ...
    await act(async () => {
      await result.current.retryModelList();
    });

    await waitFor(() => {
      expect(result.current.model_info?.current_model_id).toBe('opus-4');
    });
    expect(result.current.modelListError).toBeNull();
  });
  ...
  it('reconciles a command acknowledgement with the next runtime snapshot', async () => {
    ensureRuntimeInvokeMock.mockImplementation(async () => ({
      recovered: true,
      config_options:
        ensureRuntimeInvokeMock.mock.calls.length > 1 ? buildConfigOptions('opus-4') : buildConfigOptions('sonnet-4'),
      runtime: null,
    }));
  ...
  it('keeps the usable snapshot when a passive running-task refresh fails', async () => {
    const { result } = renderUseAcpModelInfo({
      conversation_id: 'conv-1',
      backend: 'claude',
      initialModelId: 'sonnet-4',
    ...
    await waitFor(() => expect(result.current.model_info?.current_model_id).toBe('sonnet-4'));
    ensureRuntimeInvokeMock.mockRejectedValue(new Error('runtime refresh temporarily unavailable'));
    ...
    await waitFor(() => expect(ensureRuntimeInvokeMock).toHaveBeenCalledTimes(2));
