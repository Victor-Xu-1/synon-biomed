...
const PROCESSOR_NAME = 'synon-ai-pcm-capture';
...
export const createPcmRecorder = async (options: {
  ...
  const handleWorkletMessage = (event: MessageEvent) => {
    ...
    const completeChunkCount = Math.floor(pendingSamples / chunkInputSamples);
    for (let chunkIndex = 0; chunkIndex < completeChunkCount; chunkIndex += 1) {
