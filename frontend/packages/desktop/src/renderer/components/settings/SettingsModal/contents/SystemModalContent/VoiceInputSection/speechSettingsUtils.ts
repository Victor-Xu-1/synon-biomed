...
export type SpeechSource = 'local' | 'openai' | 'deepgram' | 'custom';
...
export const migrateSpeechLanguage = (config: SpeechToTextConfig): SpeechToTextConfig => {
  ...
  if (next.local?.language === 'zh') {
    next = { ...next, local: { ...next.local, language: 'zh-CN' } };
  }
  return next;
};
...
export const applySpeechSource = (
  ...
  if (source === 'local') {
    return {
      ...config,
      provider: 'local',
      local: { ...DEFAULT_SPEECH_TO_TEXT_CONFIG.local, ...config.local },
    };
  }
  if (source === 'deepgram') {
