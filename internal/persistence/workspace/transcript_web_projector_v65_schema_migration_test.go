...
func TestTranscriptWebProjectorV65WidensVersionConstraint(t *testing.T) {
	...
	if err := applyVersionedSchemaMigrationsThrough(ctx, db, time.Now, 65); err != nil {
		t.Fatal(err)
	}
	assertTranscriptWebProjectorVersionConstraint(t, db, "CHECK(projector_versionIN(1,2,3,4,5,6,7,8,9))")
	if err := applyVersionedSchemaMigrationsThrough(ctx, db, time.Now, 65); err != nil {
		t.Fatalf("repeat v65 migration: %v", err)
	}
}
