...
import { getModelDisplayLabel } from '@/renderer/utils/synonBiomed/runtime/runtimeLogo';
import { iconColors } from '@/renderer/styles/colors';
import { Dropdown, Menu, Message, Tooltip } from '@arco-design/web-react';
import { Attention, Brain, Down, Refresh } from '@icon-park/react';
 ...
 */
const SynonBiomedModelSelector: React.FC<{
  ...
  if (modelListError) {
    ...
    return (
      <Tooltip content={hint} position='top'>
        <RuntimeSelectorPill
          testId='model-picker-degraded-hint'
          className='sendbox-model-btn header-model-btn agent-mode-compact-pill'
          aria-label={t('conversation.welcome.retryModelList')}
          label={label}
          leading={<Attention theme='outline' size='14' fill={iconColors.warning} className='shrink-0' />}
          trailing={<Refresh theme='outline' size='12' fill={iconColors.secondary} className='shrink-0' />}
          loading={isModelListLoading}
          onClick={() =>
            void retryModelList().catch((error) => {
              console.error('Failed to retry the runtime model list:', error);
            })
          }
        />
      </Tooltip>
  ...
  if (!model_info && isModelListLoading) {
    return (
      <RuntimeSelectorPill
...
export default SynonBiomedModelSelector;
