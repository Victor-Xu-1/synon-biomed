...
const warmDark = mk(WARM_DARK_THEME_ID, 'dark');
const coolLight = mk(COOL_LIGHT_THEME_ID);
...
const themes = [light, warmDark, coolLight, dark, userTheme];
...
describe('resolveActiveTheme', () => {
  ...
  it('resolves the warm system sentinel within the warm family', () => {
    expect(resolveActiveTheme(SYSTEM_THEME_ID, themes, true).id).toBe(WARM_DARK_THEME_ID);
    expect(resolveActiveTheme(SYSTEM_THEME_ID, themes, false).id).toBe(LIGHT_THEME_ID);
  });
  it('resolves the cool system sentinel within the cool family', () => {
    expect(resolveActiveTheme(COOL_SYSTEM_THEME_ID, themes, true).id).toBe(DARK_THEME_ID);
    expect(resolveActiveTheme(COOL_SYSTEM_THEME_ID, themes, false).id).toBe(COOL_LIGHT_THEME_ID);
  });
  ...
  it('falls back to Light when the warm system target is unavailable', () => {
