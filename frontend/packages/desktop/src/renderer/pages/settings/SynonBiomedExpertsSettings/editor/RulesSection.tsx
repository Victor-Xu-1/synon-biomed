...
const RulesSection: React.FC<RulesSectionProps> = ({
  ...
  return (
    <SectionCard
      title={t('settings.assistantRules')}
      legend={{
      ...
      extra={
        <div className='flex items-center gap-6px'>
          ...
          >
            {rulesExpanded ? t('common.collapse') : t('common.expand')}
          </Button>
        </div>
      }
    ...
    >
      <div
        className='overflow-hidden rounded-12px border border-arco-2 bg-fill-1'
        ...
        {promptViewMode === 'edit' && isRuleEditable ? (
          ...
          <div className='h-full overflow-auto px-14px py-12px text-13px leading-22px text-t-secondary'>
            {editContext ? (
              ...
              <div className='py-24px text-center text-t-tertiary'>{t('settings.promptPreviewEmpty')}</div>
            )}
          </div>
        )}
      </div>
