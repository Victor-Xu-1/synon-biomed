...
export interface ExcelSheetData {
  ...
  data: unknown[][]; // 单元格数据二维数组 / 2D array of cell values
...
export interface PPTSlideData {
  slideNumber: number;
  content: unknown; // PPTX JSON 结构 / PPTX JSON structure
}

export interface PPTJsonData {
  slides: PPTSlideData[];
  raw?: unknown; // 原始 PPTX JSON（可选，通常不需要传递给前端）/ Raw PPTX JSON (optional, usually not needed in frontend)
}
