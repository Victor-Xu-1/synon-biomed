...
type UseWorkspaceCollapseParams = {
   ...
   * that "send 浣犲ソ without picking a folder" leaves the panel collapsed.
   */
 ...
 * overrides auto-expand. The caller decides what `preferenceKey` is 鈥?single
 * Conversation workspaces normally use `conversation_id`.
 *
 ...
 * not trigger auto-expand on return 鈥?the user must open the panel manually
 * that one time.
 */
export function useWorkspaceCollapse({
  ...
  // A task-center action must be idempotent: opening an already-visible
  // workspace must not collapse it as a toggle would.
  useEffect(() => {
    if (typeof window === 'undefined') {
      return undefined;
    }
    const handleWorkspaceOpen = () => {
      ...
      if (preferenceKey) {
        try {
          ...
          // ignore storage errors; the in-memory state still opens the panel
        }
      }
  ...
  // The titlebar may mount before or after this hook. Responding to an
  // explicit state request prevents a user toggle from being sent before the
  // receiver is ready.
  useEffect(() => {
    if (typeof window === 'undefined') {
      return undefined;
    ...
    const handleWorkspaceStateRequest = () => {
      dispatchWorkspaceStateEvent(workspaceEnabled ? rightCollapsedRef.current : true);
    };
    window.addEventListener(WORKSPACE_STATE_REQUEST_EVENT, handleWorkspaceStateRequest);
    return () => {
      window.removeEventListener(WORKSPACE_STATE_REQUEST_EVENT, handleWorkspaceStateRequest);
    };
  }, [workspaceEnabled]);
