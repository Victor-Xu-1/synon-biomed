...
interface UseAttachEntryOptions {
  ...
  /** Receives raw browser files for a canonical project-artifact upload. */
...
export const useAttachEntry = ({
  ...
  const handleLocalFileChange = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (!fileList || fileList.length === 0 || (!onLocalFilesAdded && !onBrowserFilesAdded)) return;
      try {
        if (onBrowserFilesAdded) {
          await onBrowserFilesAdded(Array.from(fileList));
          return;
        }
    ...
    [conversationContext?.conversation_id, onBrowserFilesAdded, onLocalFilesAdded, t]
  );
