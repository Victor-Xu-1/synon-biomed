...
per-page LLM call, **in parallel**, so you load only what
...
`pypdfium2` first — the bundled runtime normally seeds it. If a call proves
that the dependency is missing, use
`manage_packages(mode="install", environment="python", packages=["pypdfium2", "pillow"], use_pip=true)`.
If the platform-managed default rejects installation, create a dedicated
Python environment with both packages and re-run there. Pillow performs PNG
...
`kernel.py` leaves `PDF_DEFAULT_MODEL = None` so calls follow the active third-party LLM profile on
every request — explicit `model=` wins over the kernel default, so the
sweep is active-profile priced (≈ $0.001–0.003/page depending on page length)
regardless of deployment config. (Ad-hoc `host.llm()` calls you make
yourself are different: bare calls use the active third-party LLM profile
