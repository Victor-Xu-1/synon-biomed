...
Do not begin with an uninformed hosted-versus-local question. Use the active
protein, antibody, or RNA design router's task and resource preflight, or
inspect the live endpoint, documented GPU/cache/disk needs, input privacy, and
credential presence without printing secrets. If one route is viable, use it.
If both are viable and data boundary, cost, setup, latency, or supported
complex size is a material user-owned trade-off, call `ask_user` once in the
conversation language and compare each route's principal advantage,
limitation, resource requirement, and structure/confidence output contract.
