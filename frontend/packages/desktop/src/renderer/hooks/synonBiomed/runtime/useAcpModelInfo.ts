...
export const useAcpModelInfo = ({
  ...
  const { model, thoughtLevel, isLoading, loadError, reload, setStatus, setConfigOption } = useAcpConfigOptions({
    conversation_id,
    prepareRuntime,
    enabled,
  });
  ...
  return {
    ...
    isModelListLoading: isLoading,
    modelListError: loadError,
    retryModelList: async () => {
      await reload();
    },
