...
const WorkspacePanelHeader: React.FC<WorkspaceHeaderProps> = ({
  ...
  const toggleLabel = t(collapsed ? 'common.expandWorkspace' : 'common.collapseWorkspace');
  ...
  return (
    ...
    >
      {showToggle && togglePlacement === 'left' && (
        <button type='button' className='workspace-header__toggle mr-4px' aria-label={toggleLabel} onClick={onToggle}>
      ...
      {showToggle && togglePlacement === 'right' && (
        <button type='button' className='workspace-header__toggle' aria-label={toggleLabel} onClick={onToggle}>
          {collapsed ? <ExpandRight size={16} /> : <ExpandLeft size={16} />}
        </button>
      )}
    </div>
...
export const DesktopWorkspaceToggle: React.FC = () => {
  ...
  return (
    <button
      ...
      aria-label={t('common.expandWorkspace')}
    >
      <ExpandLeft size={16} />
    </button>
  );
