...
export const CHAT_PREVIEW_GAP_PX = 24;
export const WORKSPACE_HEADER_HEIGHT = 32;
export const MIN_CHAT_PANEL_PX = 360;
export const MIN_PREVIEW_PANEL_PX = 340;
// The file rail remains fully usable at 176px: its search, toolbar and
// responsive 70px artifact tracks still fit, while users can recover more
// horizontal room for the conversation by dragging the divider farther right.
export const MIN_WORKSPACE_PANEL_PX = 176;
// Files, compute, and notebooks need a substantive right-hand workspace.
// Keep a usable default
// while still allowing a wide, user-resizable analysis pane on large screens.
export const MAX_WORKSPACE_PANEL_PX = 1200;
export const DEFAULT_WORKSPACE_PANEL_PX = 480;
// The previous preference key was populated while the right workspace was a
// narrow utility rail. Its stored pixel values are not valid preferences for
// the current workbench, so use a new preference epoch and start at the
// verified 480px default once after upgrade.
export const WORKSPACE_WIDTH_STORAGE_KEY = 'chat-workspace-width-px:claude-science-v1';
...
export const calcLayoutMetrics = (input: LayoutCalcInput): LayoutMetrics => {
  ...
  const previewReservedPx = isPreviewOpen
    ? MIN_CHAT_PANEL_PX + MIN_PREVIEW_PANEL_PX + CHAT_PREVIEW_GAP_PX
    : MIN_CHAT_PANEL_PX;
  ...
  const availableWidthForChatPreview = Math.max(
    safeContainerWidth - effectiveWorkspaceWidthPx - (isPreviewOpen ? CHAT_PREVIEW_GAP_PX : 0),
    1
  );
