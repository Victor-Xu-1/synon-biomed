...
export function useLayoutConstraints({
  ...
  useEffect(() => {
    ...
    const maxWorkspaceByContainer = Math.max(
      MIN_WORKSPACE_PANEL_PX,
      safeContainerWidth - MIN_CHAT_PANEL_PX - MIN_PREVIEW_PANEL_PX - CHAT_PREVIEW_GAP_PX
    );
    ...
    if (safeContainerWidth < MIN_CHAT_PANEL_PX + MIN_PREVIEW_PANEL_PX + CHAT_PREVIEW_GAP_PX + MIN_WORKSPACE_PANEL_PX) {
