...
import SynonBiomedArtifactLibrary from './components/SynonBiomedArtifactLibrary';
...
const ChatWorkspace: React.FC<WorkspaceProps> = ({
  ...
  const showComputeTab = isSynonBiomedWorkspace && Boolean(rootFrameId);
  const isReadOnlyWorkspace = isSynonBiomedWorkspace;
  ...
  const fileChangesHook = useFileChanges({ workspace, enabled: !isReadOnlyWorkspace });
  ...
  const treeHook = useWorkspaceTree({ workspace, conversation_id, projectId, eventPrefix });
  ...
  useWorkspaceEvents({
    conversation_id,
    eventPrefix,
    ensureWorkspace: treeHook.ensureWorkspace,
    refreshWorkspace: treeHook.refreshWorkspace,
  ...
  // synon-ai-conversation::convert::row_to_response). We never inspect the
  ...
  const handleViewArtifactDetails = useCallback(
    (node: IDirOrFile) => {
      if (!node.artifactId) return;
      void navigate(`/artifacts/${encodeURIComponent(node.artifactId)}`);
    },
    [navigate]
  );
  ...
  return (
    <>
      ...
      <div
        ...
        {...(isReadOnlyWorkspace ? {} : dragImportHook.dragHandlers)}
        style={
          !isReadOnlyWorkspace && dragImportHook.isDragging
            ? {
      ...
      >
        {!isReadOnlyWorkspace && dragImportHook.isDragging && (
          <div className='absolute inset-0 pointer-events-none z-30 flex items-center justify-center px-32px'>
            ...
            >
              <div className='text-18px font-semibold mb-8px'>{t('conversation.workspace.dragOverlayTitle')}</div>
              <div className='text-14px opacity-90 mb-4px'>{t('conversation.workspace.dragOverlayDesc')}</div>
              <div className='text-12px opacity-70'>{t('conversation.workspace.dragOverlayHint')}</div>
            </div>
          </div>
        )}
        ...
        <WorkspaceTabBar
          ...
          showCompute={showComputeTab}
        />

        {/* Toolbar: search input + directory name + action buttons */}
        {activeTab === 'files' && !isSynonBiomedWorkspace && (
          <WorkspaceToolbar
            t={t}
            ...
            readOnly={isReadOnlyWorkspace}
            allowUpload={isSynonBiomedWorkspace || !isReadOnlyWorkspace}
        ...
        {!isWorkspaceCollapsed && activeTab === 'files' && (
          <FlexFullContainer containerClassName='overflow-y-auto'>
            ...
            <WorkspaceContextMenu
              ...
              handleViewArtifactDetails={handleViewArtifactDetails}
            ...
            {isSynonBiomedWorkspace ? (
              <SynonBiomedArtifactLibrary
                nodes={treeHook.files}
                loading={treeHook.loading}
                total={treeHook.projectArtifactPage.total}
                hasMore={treeHook.projectArtifactPage.hasMore}
                loadingMore={treeHook.projectArtifactPage.loadingMore}
                loadMoreError={treeHook.projectArtifactPage.loadMoreError}
                onRefresh={() => void treeHook.refreshWorkspace()}
                onSearch={(query) => treeHook.searchWorkspace(query)}
                onLoadMore={() => treeHook.loadMoreWorkspace()}
                onOpen={(artifact) => void fileOpsHook.handlePreviewFile(artifact, treeHook.files)}
                onMenu={(artifact, x, y) => openNodeContextMenu(artifact, x, y)}
              />
            ) : !hasOriginalFiles ? (
        ...
        {activeTab === 'compute' && showComputeTab && rootFrameId && (
