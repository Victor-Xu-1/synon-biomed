...
const SpeechInputButton: React.FC<SpeechInputButtonProps> = ({ disabled, onLiveTranscript, onTranscript }) => {
  ...
  const isProcessing = isStarting || status === 'transcribing';
  ...
  return (
    <>
      ...
      <div className={`speech-input-control ${showSpeechFeedback ? 'speech-input-control--active' : ''}`}>
        {showSpeechFeedback && (
          <div
            className={`speech-input-feedback ${isStarting ? 'speech-input-feedback--starting' : ''} ${isProcessing ? 'speech-input-feedback--processing' : ''}`}
            role='status'
            aria-live='polite'
          >
            ...
            <span className='speech-input-feedback__label'>
              {isStarting
                ? t('conversation.chat.speech.startShort')
                : status === 'transcribing'
