...
Credential enrollment is an external user-owned action. Do not create or copy
a key automatically; stop for the approved cluster credential path.
...
- **Fix**: use the user's existing approved `ssh-agent` session or another
  cluster-approved non-interactive credential. Do not weaken an existing key or
  create a passphrase-less replacement automatically.
...
- **Fix**: prefer a new unique `run_name`, `RUN_SUFFIX`, target, or count. If
  the old remote run truly must be removed, resolve and inspect the exact path,
  preserve required results, obtain explicit approval, and use the cluster's
  documented cleanup procedure; never interpolate a recursive delete command.
...
- **Fix**: `acquire_lock` (`slurm_helper.sh:361-378`) reclaims locks whose
  owner PID no longer exists. If manual recovery is still required, inspect the
  recorded owner and remove only an empty, verified lock directory; do not use
  recursive deletion against the repository root.
