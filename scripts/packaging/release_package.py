#!/usr/bin/env python3
"""Publish verified release files as a content-addressed OCI distribution bundle."""
    ...
    while True:
        ...
        for version in versions:
            if tag in version.get("metadata", {}).get("container", {}).get("tags", []):
                raise ValueError("This package version already exists; do not overwrite a published version")
        if len(versions) < 100:
            return
        page += 1
...
def main() -> None:
    if os.environ.get("GITHUB_EVENT_NAME") != "release":
        raise ValueError("Package publication only accepts the release event")
    ...
    with tempfile.TemporaryDirectory(prefix="synon-package-") as temp:
        ...
        verify_roundtrip(artifacts, root / "local-pull", f"{layout}@{digest}", local=True)
        registry_config = str(root / "registry-auth.json")
        subprocess.run(
            ["oras", "login", "ghcr.io", "--registry-config", registry_config,
        ...
        verify_roundtrip(artifacts, root / "remote-pull",
                         f"ghcr.io/{REPOSITORY.lower()}@{digest}", registry_config=registry_config)
        with Path(os.environ["GITHUB_STEP_SUMMARY"]).open("a") as summary:
            summary.write(f"## Published OCI release bundle\n\n- Package: `{remote}`\n- Digest: `{digest}`\n- Source: `{revision}`\n\nThis is an installation bundle, not a runnable container.\n")
        print(f"Published and reverified {remote}@{digest}")
