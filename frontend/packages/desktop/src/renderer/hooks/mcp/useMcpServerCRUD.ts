...
const getMcpRequestErrorDiagnostic = (error: unknown): Record<string, unknown> => {
  if (isBackendHttpError(error)) {
    return { name: error.name, status: error.status, code: error.code || undefined };
  }
  ...
  const handleAddMcpServer = useCallback(
    async (serverData: Omit<IMcpServer, 'id' | 'created_at' | 'updated_at'>) => {
      try {
        ...
        console.error('Failed to add MCP server:', getMcpRequestErrorDiagnostic(error));
    ...
    async (serversData: Omit<IMcpServer, 'id' | 'created_at' | 'updated_at'>[]) => {
      try {
        ...
        const finalServers = await Promise.all(
          imported.map(async (importedServer): Promise<IMcpServer> => {
            ...
            return mergeServerState(persisted, original);
          })
        );
        ...
        console.error('Failed to import MCP servers:', getMcpRequestErrorDiagnostic(error));
        Message.error(t('settings.mcpImportFailed'));
        return [];
      }
    },
  ...
  const handleEditMcpServer = useCallback(
    async (
      ...
      try {
        ...
        console.error('Failed to update MCP server:', getMcpRequestErrorDiagnostic(error));
        Message.error(t('settings.mcpImportFailed'));
        return undefined;
      }
    },
