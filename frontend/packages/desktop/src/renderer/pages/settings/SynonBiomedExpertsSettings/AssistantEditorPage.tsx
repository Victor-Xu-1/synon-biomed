  ...
  return (
    <div data-testid='assistant-editor-page' className='flex h-full min-h-0 flex-col overflow-hidden bg-transparent'>
      <div
        ...
        className='sticky top-0 z-10 flex h-48px flex-shrink-0 items-center gap-12px border-b border-arco-2 bg-base px-18px'
      >
        <div className='flex min-w-0 items-center gap-10px'>
          ...
          >
            {t('settings.assistantBackToList')}
          </Button>
          <div className='truncate text-14px font-600 text-t-primary'>
            {profile.name.trim() || (isCreating ? t('settings.createAssistant') : t('settings.editAssistant'))}
          </div>
        </div>
