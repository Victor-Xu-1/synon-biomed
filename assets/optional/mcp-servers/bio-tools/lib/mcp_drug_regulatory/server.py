"""mcp-drug-regulatory server — MCPServer tools over openfda-drugsfda + openfda-labels.
...
from mcp_servers_common.criteria import blank
...
mcp = MCPServer("mcp-drug-regulatory")
...
def search_drug_applications(
    ...
    """
    if blank(raw_search):
        raise ValueError("raw_search must be non-empty")
    spec = _build_spec(
...
def search_drug_labels(
    ...
    """Retrieve authoritative FDA drug product labels (SPL) by ingredient/name/route.

    Use this as the source-specific lookup for prescribing information, drug
    label parameters, clinical pharmacology, pharmacokinetics / PK,
    pharmacodynamics, absorption, distribution, metabolism, elimination,
    dosage, safety, warnings, contraindications, and adverse reactions.
    ...
    Args:
        ...
        max_records: Positive cap on label records retrieved and returned.
            The API's authoritative total count is still preserved and
            `truncated` reports whether more records matched.
    ...
    if isinstance(max_records, bool) or not 1 <= max_records <= 1000:
        raise ValueError("max_records must be in [1, 1000]")
    result = run_spec(spec, _labels(), sections=sections, max_records=max_records)
    out = result["records"]
    return {
