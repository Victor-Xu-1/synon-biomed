...
molecular-generation router's task and resource preflight, or inspect the live
endpoint, local GPU/cache/disk readiness, and credential presence without
printing secrets. If one route is viable, use it. If both are viable and data
boundary, cost, setup, latency, or access to latent endpoints is a material
