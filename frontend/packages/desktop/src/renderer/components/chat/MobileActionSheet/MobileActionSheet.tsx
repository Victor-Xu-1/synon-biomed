...
const MobileActionSheet: React.FC<MobileActionSheetProps> = ({ open, onClose, title, entries }) => {
  ...
  useEffect(() => {
    ...
    const handleEscape = (event: KeyboardEvent) => {
      ...
      if (activeSubKey) setActiveSubKey(null);
      else onClose();
    };
    window.addEventListener('keydown', handleEscape);
    return () => window.removeEventListener('keydown', handleEscape);
  }, [activeSubKey, onClose, open]);
            ...
            <div className={styles.list} role='menu'>
              {entries.map((entry, index) => (
                <Fragment key={entry.key}>
                  {entry.dividerBefore && index !== 0 && <div className={styles.divider} role='separator' />}
                  <button
                    type='button'
                  ...
                  renderedSub.options.map((option) => {
                    ...
                    return (
                      <button
                        ...
                        role={showRadio ? 'menuitemradio' : 'menuitem'}
                        aria-checked={showRadio ? Boolean(option.active) : undefined}
