...
import { classifyConfigSetError, useAcpConfigOptions } from '@/renderer/hooks/synonBiomed/runtime/useAcpConfigOptions';
import type { AgentModeOption } from '@/renderer/utils/synonBiomed/runtime/runtimeTypes';
import { useLayoutContext } from '@/renderer/hooks/context/LayoutContext';
import { SynonBiomedRuntimeLogoIcon } from './SynonBiomedRuntimeBadge';
...
export interface SynonBiomedModeSelectorProps {
 ...
 */
const SynonBiomedModeSelector: React.FC<SynonBiomedModeSelectorProps> = ({
  backend,
  agent_name,
  agentLogo,
  agentLogoIsEmoji,
  ...
  const getDisplayModeLabel = useCallback(
    (mode: AgentModeOption) => {
      if (modeLabelFormatter) return modeLabelFormatter(mode);
      const key = `agentMode.${mode.value}`;
      return i18n.exists(key) ? t(key) : mode.label;
    },
    [i18n, modeLabelFormatter, t]
  );
  ...
  const handleModeChange = useCallback(
    async (mode: string) => {
      ...
      try {
        ...
        console.error('[SynonBiomedModeSelector] Failed to switch mode:', error);
        ...
        {modes.map((mode: AgentModeOption) => (
          <Menu.Item key={mode.value} className={current_mode === mode.value ? '!bg-2' : ''}>
            <div
              ...
              data-testid={`conversation-mode-option-${mode.value}`}
            >
  ...
  if (compact) {
    ...
    const fallbackAgentLabel = t('common.agent');
    const baseCompactLabel =
      compactLabelType === 'agent'
        ? agent_name || backend || fallbackAgentLabel
        : can_switchMode
          ? getCurrentModeLabel()
          : agent_name || backend || fallbackAgentLabel;
    const compactLabel =
...
export default SynonBiomedModeSelector;
