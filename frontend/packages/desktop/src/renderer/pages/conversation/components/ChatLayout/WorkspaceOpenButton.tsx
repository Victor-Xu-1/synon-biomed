...
const WorkspaceOpenButton: React.FC<WorkspaceOpenButtonProps> = ({ workspacePath, isTemporary }) => {
  ...
  useEffect(() => {
    if (!isElectronDesktop() || isTemporary) return;
  ...
  const toolOptions: ToolOption[] = [
    {
      key: 'vscode',
      label: t('conversation.workspace.openWith.vscode'),
      icon: <Command size={16} />,
      available: vscodeInstalled,
    },
    {
      key: 'terminal',
      label: t('conversation.workspace.openWith.terminal'),
      icon: <Terminal size={16} />,
      available: true,
    },
    {
      key: 'explorer',
      label: t('conversation.workspace.openWith.explorer'),
      icon: <Folder size={16} />,
      available: true,
    },
  ];
  ...
  const dropdownList = (
    <div
      className='app-overlay-menu workspace-open-dropdown p-4px'
      role='menu'
      aria-label={t('conversation.workspace.openWorkspace')}
    >
      {availableOptions.map((option) => (
        <button
          ...
          type='button'
          role='menuitemradio'
          aria-checked={currentTool === option.key}
          className={`workspace-open-dropdown-item flex w-full items-center gap-8px border-0 bg-transparent px-12px py-8px text-left cursor-pointer hover:bg-[var(--color-fill-2)] rounded-4px transition-colors ${
            currentTool === option.key ? 'bg-[var(--color-fill-2)]' : ''
          }`}
          onClick={() => void handleOpenWith(option.key)}
        >
  ...
  return (
    <div className='workspace-open-button flex items-center'>
      <Tooltip content={t('conversation.workspace.openWorkspace')} mini>
        <Button
          type='text'
          size='small'
