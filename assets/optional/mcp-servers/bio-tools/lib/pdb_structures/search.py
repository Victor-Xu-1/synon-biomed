    ...
    *,
    ...
    if taxonomy_id is not None:
        nodes.append(_text_node(
            ...
            "exact_match", str(int(taxonomy_id))))
...
def search_structures(
    ...
    *,
    ...
    Returns ``{"total_count", "n_retrieved", "truncated", "sort_by",
    "sort_direction", "records"}`` where records are ``{"pdb_id", "score"}``
    in the requested server-owned order. ``truncated`` is
