...
def _get_metadata(client: BiomartClient, params: dict[str, str]) -> str:
    ...
    # Registry/configuration payloads are small. Bound each metadata attempt
    # tightly so list_marts can return its audited fallback during an Ensembl
    # outage instead of being killed by the outer MCP deadline.
    metadata_timeout = min(client.timeout, 12.0)
    for attempt in range(client.max_retries + 1):
        ...
        try:
            resp = client._session.get(
                client.martservice_url, params=params, timeout=metadata_timeout,
