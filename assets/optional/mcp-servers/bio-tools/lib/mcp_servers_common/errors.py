...
One owned predicate (``is_error_payload``) + one MCPServer decorator
...
- **Tier-1** (``Tier1Server._call_tool``) sets ``isError=True`` on the
  ...
  payload.
- **Tier-2 / MCPServer** tools that can return error dicts are decorated with
  ``@raise_on_error_payload`` (under ``@mcp.tool(...)``); the raise reaches
  MCPServer's ``Tool.run`` catch-all, which surfaces it as ``isError=True``
...
def raise_on_error_payload(fn):
    """MCPServer tool decorator: an ``{"error": ...}`` return raises
    ``ToolError`` instead of returning on the success path.

    Apply UNDER ``@mcp.tool(...)`` — ``functools.wraps`` preserves the
    signature MCPServer introspects for input/output schema generation (via
