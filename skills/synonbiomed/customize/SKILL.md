...
- **`system_prompt`** — the profile's **identity**. This is the opening of the
  agent's system prompt; it REPLACES the generic "You are Synon Biomed" base identity.
...
#   origin: "synon_llm" (bundled, read-only — fork under a new name),
...
# cache; synon_llm bundled → protected.
...
After the profile exists, propose a managed environment for it: name it after
the profile (lowercase slug), pick `python` or `r` from the attached Skills,
and list the required packages. Ask the user to confirm the package list, then
call `manage_environments(mode="list", dependencies=[...])` and reuse a
compatible environment when possible. Otherwise call
`manage_environments(mode="create", name="<slug>", packages=[...])`; add any
later missing dependency with
`manage_packages(mode="install", environment="<slug>", packages=[...])`.
Run Python, R, or Bash only with that verified environment.
...
- `import` / `from … import name` (no `*`). Defer third-party imports to
  ...
  traceback and must move the dependency into a reviewed execution pack. Do not
  reconstruct the retired model-authored package/script request.
