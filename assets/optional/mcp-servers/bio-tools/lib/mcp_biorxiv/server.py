...
def _limit(args: dict) -> int:
    # Discovery should expose a useful candidate pool by default. Callers can
    # still request a smaller page for an exact lookup, while cursor-based
    # continuation remains the authority for exhaustive retrieval.
    return max(1, min(100, int(args.get("limit", 50))))
