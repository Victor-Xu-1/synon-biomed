...
async def search_cell_types(
    ...
    Args:
        ...
        limit: Maximum number of results in this page (1-200)
        offset: Zero-based offset into ranked matches

    Returns ``{result: [{id, name, synonyms, ontology_description}, ...]}``.
    """
    if blank(query):
        raise ValueError("query must be non-empty")
    page_method = getattr(_client, "search_cell_types_page", None)
    if callable(page_method):
        matches, total = await page_method(query, limit=limit, offset=offset)
    else:
        # Compatibility for existing injected clients: the public server
        # contract gains pagination without changing their list-returning API.
        all_matches = await _client.search_cell_types(query, limit=limit + offset)
    ...
    # Wrapped in a dict so MCPServer serializes ONE JSON object — a bare
    # ``list[dict]`` return is per-item-serialized into N TextContent blocks
    # (MCPServer's ``_convert_to_content`` recurses into lists), which the
    # 06-25 probe saw as "a raw string of multiple JSON blobs" vs the
    # documented ``{result: array<object>}`` schema.
    next_offset = max(0, int(offset)) + len(results)
    return {
        "result": results,
        "total_matches": total,
