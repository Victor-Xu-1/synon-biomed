      ...
      try {
        ...
        if (absolutePath || originalPath) {
          try {
            ...
            const binaryMimeType = getLocalBinaryPreviewMimeType(computedFileName || previewTitle, contentType);
            if (binaryMimeType) {
              const encoded = await ipcBridge.fs.readFileBuffer.invoke({ path: pathToRead!, workspace });
              if (!encoded) {
                setErrorKind(classifyPreviewError(encoded));
                return;
              }
              openPreview(
                '',
                ...
                {
                  ...metadata,
                  contentUrl: buildBase64PreviewDataUrl(encoded, binaryMimeType),
                  editable: false,
                },
                { presentation: 'board' }
              );
            ...
            const binaryOnlyTypes: PreviewContentType[] = [
              'pdf',
              'ppt',
              'word',
              'excel',
