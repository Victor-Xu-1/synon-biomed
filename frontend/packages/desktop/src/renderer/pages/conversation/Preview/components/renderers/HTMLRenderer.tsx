...
interface HTMLRendererProps {
  ...
  elementAnnotations?: HtmlElementAnnotationMarker[];
  onElementAnnotationClick?: (annotationId: string) => void;
  onTextSelection?: (selection: HtmlTextSelection | null) => void;
}
...
async function inlineRelativeResources(html: string, basePath: string, workspace?: string): Promise<string> {
  ...
  const imageReplacements = await Promise.all(
    imgMatches.map(async (match): Promise<readonly [string, string] | null> => {
  ...
  imageReplacements.forEach((replacement) => {
    if (replacement) result = result.replace(replacement[0], replacement[1]);
  });
  ...
  const linkReplacements = await Promise.all(
    linkMatches.map(async (match): Promise<readonly [string, string] | null> => {
      ...
      if (isStylesheet) {
        try {
          ...
          if (cssContent) {
            ...
            const cssReplacements = await Promise.all(
              cssUrlMatches.map(async (urlMatch): Promise<readonly [string, string] | null> => {
                ...
                try {
                  ...
                  if (dataUrl) {
                    return [urlFullMatch, `url("${dataUrl}")`];
                  }
                } catch (e) {
            ...
            cssReplacements.forEach((replacement) => {
              if (replacement) processedCss = processedCss.replace(replacement[0], replacement[1]);
            });

            const styleTag = `<style>${processedCss}</style>`;
            return [fullMatch, styleTag];
          }
        } catch (e) {
  ...
  const scriptReplacements = await Promise.all(
    scriptMatches.map(async (match): Promise<readonly [string, string] | null> => {
      const [fullMatch, before, src, after] = match;
      try {
        const absolutePath = resolveRelativePath(basePath, src);
...
const HTMLRenderer: React.FC<HTMLRendererProps> = ({
  ...
  const copySuccessText = useMemo(() => copySuccessMessage ?? t('preview.html.copySuccess'), [copySuccessMessage, t]);
  const inspectScript = useMemo(
    () => generateInspectScript(inspectMode, { copySuccess: copySuccessText }),
    [inspectMode, copySuccessText]
  );
  const annotationHighlightScript = useMemo(
    () => generateHtmlAnnotationHighlightScript(elementAnnotations),
    [elementAnnotations]
  );
  ...
  const textSelectionScript = useMemo(
    () => `
      (function() {
        if (window.__synonAiTextSelectionInitialized) return;
        window.__synonAiTextSelectionInitialized = true;
        document.addEventListener('mouseup', function() {
          setTimeout(function() {
            const selection = window.getSelection();
            const text = selection && !selection.isCollapsed ? selection.toString().trim() : '';
            if (!text || !selection || selection.rangeCount === 0) {
              console.log('__TEXT_SELECTION__null');
              return;
            }
            const rect = selection.getRangeAt(0).getBoundingClientRect();
            console.log('__TEXT_SELECTION__' + JSON.stringify({
              text: text,
  ...
  useEffect(() => {
    const webview = webviewRef.current;
    if (!webview || !onTextSelection) return;
    const inject = () => {
      void webview.executeJavaScript(textSelectionScript).catch(() => {});
    };
    if (webviewLoadedRef.current) inject();
    webview.addEventListener('did-finish-load', inject);
    return () => webview.removeEventListener('did-finish-load', inject);
  }, [onTextSelection, textSelectionScript, webviewSrc]);
      ...
      if (typeof message === 'string') {
        ...
        if (message.startsWith('__INSPECT_ELEMENT__') && onElementSelected) {
          try {
            ...
            onElementSelected(mapInspectedElementToHost(data, webview.getBoundingClientRect()));
          } catch (e) {
        ...
        } else if (message.startsWith('__TEXT_SELECTION__') && onTextSelection) {
          try {
            const raw = message.slice('__TEXT_SELECTION__'.length);
            if (raw === 'null') {
              onTextSelection(null);
            } else {
              const data = JSON.parse(raw) as HtmlTextSelection;
              const rect = webview.getBoundingClientRect();
              onTextSelection({ text: data.text, x: rect.left + data.x, y: rect.top + data.y });
            }
          } catch (e) {
            console.warn('[HTMLRenderer] Failed to parse text selection message:', e);
          }
        } else if (message.startsWith('__HTML_ANNOTATION_CLICK__') && onElementAnnotationClick) {
          onElementAnnotationClick(message.slice('__HTML_ANNOTATION_CLICK__'.length));
        }
  ...
  }, [onElementAnnotationClick, onElementSelected, onScroll, onTextSelection]);

  useEffect(() => {
    const webview = webviewRef.current;
    if (!webview) return;
    const inject = (): void => {
      void webview.executeJavaScript(annotationHighlightScript).catch(() => {});
    };
    if (webviewLoadedRef.current) inject();
    webview.addEventListener('did-finish-load', inject);
    return () => webview.removeEventListener('did-finish-load', inject);
  }, [annotationHighlightScript, webviewSrc]);
...
function mapInspectedElementToHost(element: InspectedElement, hostRect: DOMRect): InspectedElement {
  const rect = element.rect;
  if (!rect) return element;
  const scaleX = hostRect.width / Math.max(1, rect.viewportWidth);
  const scaleY = hostRect.height / Math.max(1, rect.viewportHeight);
  return {
    ...element,
    anchorX: hostRect.left + (rect.x + rect.width / 2) * scaleX,
    anchorY: hostRect.top + (rect.y + rect.height) * scaleY,
