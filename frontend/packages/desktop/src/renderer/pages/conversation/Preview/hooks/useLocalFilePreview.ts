...
const shouldReadPreviewContent = (contentType: PreviewContentType): boolean =>
  !['pdf', 'word', 'excel', 'ppt', 'audio', 'video', 'hdf5', 'unsupported'].includes(contentType);
  ...
  return useCallback(
    async (file_path: string, reference?: LocalFileLinkReference) => {
      ...
      try {
        ...
        const binaryMimeType = getLocalBinaryPreviewMimeType(fileName, contentType);
        if (binaryMimeType) {
          const encoded = await ipcBridge.fs.readFileBuffer.invoke({ path: file_path, workspace });
