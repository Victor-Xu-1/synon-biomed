...
describe('deriveSpeechSource', () => {
  ...
  it('returns local for the offline provider', () => {
    const config = normalizeSpeechToTextConfig({ enabled: true, provider: 'local' });
    expect(deriveSpeechSource(config)).toBe('local');
    expect(config.local?.model).toBe('sherpa-onnx-streaming-zipformer-small-ctc-zh-int8-2025-04-01');
  });
...
describe('applySpeechSource', () => {
  ...
  it('switching to local selects the fixed bundled-compatible model without API settings', () => {
    const next = applySpeechSource(customConfig, 'local');
    expect(next.provider).toBe('local');
    expect(next.local?.model).toBe('sherpa-onnx-streaming-zipformer-small-ctc-zh-int8-2025-04-01');
    expect(next.openai?.api_key).toBe('k');
  });
...
describe('model presets', () => {
  ...
  it('defaults use the recommended models', () => {
    ...
    expect(DEFAULT_SPEECH_TO_TEXT_CONFIG.local?.language).toBe('zh-CN');
    expect(DEFAULT_SPEECH_TO_TEXT_CONFIG.local?.model).toBe(
...
describe('migrateSpeechLanguage', () => {
  ...
  it('migrates local zh to zh-CN', () => {
    const config = normalizeSpeechToTextConfig({
      enabled: true,
      provider: 'local',
      local: { language: 'zh', model: 'sherpa-onnx-streaming-zipformer-small-ctc-zh-int8-2025-04-01' },
    });
    expect(migrateSpeechLanguage(config).local?.language).toBe('zh-CN');
  });
