...
allowed-tools: search_skills, skill, ask_user, repl, list_compute, bash, read_file, edit_file, save_artifacts
---
...
Load `complexa-setup` for local CLI/config/weight readiness. SLURM submission
additionally needs cluster reachability, an `sbatch` binary, a live partition,
and an approved account/quota. Run the real task-scoped cluster preflight after
local readiness; no shared preflight helper is bundled here.

```bash
bash "${SYNON_SKILL_DIR}/scripts/cluster_preflight.sh" --out ./cluster_preflight.json
```
...
## Step 3: Resolve parameters and material choices

Resolve routine values from the selected Complexa route, checked config, live
cluster preflight, and user request. Relevant fields include:
...
When two to four viable runtime, sweep, target-set, quota, or download choices
materially change scientific coverage, cost, latency, data movement, or
deliverables, call `ask_user` once after preflight. Recommend one and describe
each option's advantage, limitation, requested cluster resources, and expected
outputs in the conversation language. Do not ask the user to diagnose SSH,
partition, or environment failures.

## Step 4: Dry-run before external submission

Build the command and run it with `--dry-run`. Summarize the resolved workload,
Hydra overrides, data destination, nodes/GPUs/time/partition/account, array
size, and estimated quota or cost. Preserve the full generated sbatch body as
internal evidence and show it only when the user asks to audit it. External
submission requires the corresponding user-owned approval.
...
## Step 8: Preserve reproducibility evidence

Preserve an internal durable record with the exact launch command, immutable
code/weights, cluster host identity, partition/account, runtime, resolved run
directory, generated sbatch files and hashes, job IDs, submission receipts, and
downloaded result paths. Publish a user-facing audit artifact only when it is
requested or materially useful; otherwise keep internal cluster paths and
mechanics out of the default file panel.
...
The launch scripts already write `.githash` into the remote run directory;
retain that SHA in the durable evidence and join every downloaded result to its
job and stage.
...
| `Directory ... already exists on remote` | A prior run with the same `run_name` is on the cluster | Prefer a new unique `run_name`. If cleanup is explicitly approved, inspect the resolved path and use the cluster's documented cleanup procedure; never paste an interpolated recursive deletion command into an SSH one-liner. |
