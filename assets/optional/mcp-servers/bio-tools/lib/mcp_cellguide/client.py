...
class CellGuideClient:
    ...
    async def search_cell_types(self, query: str, limit: int = 30) -> list[dict[str, Any]]:
        """Return the first ranked page, preserving the original client API."""
        page, _total = await self.search_cell_types_page(query, limit=limit)
        return page

    async def search_cell_types_page(
        ...
        Args:
            ...
            limit: Maximum results to return in this page
            offset: Zero-based offset into the ranked matches

        Returns:
            Page of matching metadata and the total ranked match count
        """
        metadata = await self.get_celltype_metadata()
        ...
        return [info for _, info in matches[offset:offset + limit]], len(matches)
