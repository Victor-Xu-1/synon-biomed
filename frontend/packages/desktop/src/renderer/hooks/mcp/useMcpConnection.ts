...
const formatMcpErrorMessage = (t: TFunction, payload: McpErrorPayload): string => {
  ...
  switch (payload.code) {
    ...
    case 'MCP_CONNECTION_FAILED':
      return t('settings.mcpErrorConnectionFailed');
    case 'MCP_HTTP_ERROR':
    ...
    case 'MCP_PROTOCOL_ERROR':
      return t('settings.mcpErrorProtocol');
    default:
    ...
    async (server: IMcpServer, options?: TestOptions) => {
      ...
      try {
        ...
        if (needsAuth) {
          ...
          if (notify) {
            await globalMessageQueue.add(() => {
              try {
                message.warning(`${server.name}: ${t('settings.mcpAuthRequired')}`);
              } catch {
        ...
        if (result.success) {
          ...
          console.warn('[MCP] Connection test failed', { serverId: server.id, code: result.code });
        ...
        // Record the latest failed availability test in local UI state.
        await updateServerStatus('error');
        console.error('[MCP] Connection test threw an error:', error);
