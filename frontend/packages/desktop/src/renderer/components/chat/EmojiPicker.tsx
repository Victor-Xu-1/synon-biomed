...
const EMOJI_CATEGORIES = {
  recent: {
    icon: '🕐',
    labelKey: 'settings.emojiCategories.recent',
    emojis: [] as string[], // Will be populated from localStorage
  },
  smileys: {
    icon: '😀',
    labelKey: 'settings.emojiCategories.smileys',
    emojis: [
      '😀',
      '😃',
      '😄',
  ...
  animals: {
    icon: '🐱',
    labelKey: 'settings.emojiCategories.animals',
    emojis: [
      '🐶',
      '🐱',
      '🐭',
  ...
  food: {
    icon: '🍎',
    labelKey: 'settings.emojiCategories.food',
    emojis: [
      '🍎',
      '🍐',
      '🍊',
  ...
  activities: {
    icon: '⚽',
    labelKey: 'settings.emojiCategories.activities',
    emojis: [
      '⚽',
      '🏀',
      '🏈',
  ...
  objects: {
    icon: '💡',
    labelKey: 'settings.emojiCategories.objects',
    emojis: [
      '💡',
      '🔦',
      '🏮',
  ...
  symbols: {
    icon: '❤️',
    labelKey: 'settings.emojiCategories.symbols',
    emojis: [
      '❤️',
      '🧡',
      '💛',
  ...
  flags: {
    icon: '🏁',
    labelKey: 'settings.emojiCategories.flags',
    emojis: [
      '🏁',
      '🚩',
      '🎌',
...
const RECENT_EMOJIS_KEY = 'synon-ai.emoji.recent';
    ...
    <div className='w-280px'>
      ...
      <div className='flex items-center gap-2px px-8px py-6px border-b border-[var(--color-border-2)] overflow-x-auto'>
        {categoryKeys.map((key) => (
          <button
            ...
            aria-label={t(EMOJI_CATEGORIES[key].labelKey)}
            title={t(EMOJI_CATEGORIES[key].labelKey)}
          >
      ...
      <div className='p-8px max-h-200px overflow-y-auto'>
        {currentEmojis.length > 0 ? (
          ...
          <div className='text-center text-t-secondary py-16px text-14px'>{t('settings.noRecentEmojis')}</div>
        )}
      </div>
    </div>
  );
  ...
  const builtinAvatarContent = (
    <div className='w-280px p-8px max-h-264px overflow-y-auto'>
      {builtinAvatars.length > 0 ? (
        ...
        <Empty description={t('settings.assistantAvatarNoBuiltinImages')} />
      )}
    </div>
  );
      ...
      <div className='w-280px'>
        <Tabs activeTab={activeTab} onChange={(key) => setActiveTab(key as 'emoji' | 'builtin')} size='small'>
          <Tabs.TabPane
            ...
            title={
              <span className='flex items-center gap-4px'>
                <span aria-hidden='true'>👤</span> <span>{t('settings.assistantAvatarBuiltinTab')}</span>
              </span>
            }
          >
          ...
          <Tabs.TabPane
            ...
            title={
              <span className='flex items-center gap-4px'>
                <span aria-hidden='true'>🙂</span> <span>{t('settings.assistantAvatarEmojiTab')}</span>
              </span>
            }
          >
            {emojiPickerContent}
