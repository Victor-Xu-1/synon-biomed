...
def _fetcher():
    ...
    return PubMedFetcher(email=_contact_email(), tool="mcp-pubmed", api_key=NCBI_API_KEY)


@lru_cache(maxsize=1)
def _search():
    from pubmed_search import PubMedSearch
    return PubMedSearch(email=_contact_email(), tool="mcp-pubmed", api_key=NCBI_API_KEY)
...
def _citmatch_search():
    ...
    from pubmed_search import PubMedSearch
    return PubMedSearch(
        email=_contact_email(), tool="mcp-pubmed", api_key=NCBI_API_KEY,
