...
first="$(find "$tmp/first" -maxdepth 1 -type f -name "${product_slug}-v${product_version}-windows-amd64.tar.gz" -print -quit)"
test -n "$first"
mkdir -p "$tmp/unpack"
...
second="$(find "$tmp/second" -maxdepth 1 -type f -name "${product_slug}-v${product_version}-windows-amd64.tar.gz" -print -quit)"
