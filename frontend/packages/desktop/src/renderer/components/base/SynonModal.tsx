...
/** SynonModal 组件 Props */
export interface SynonModalProps extends Omit<ModalProps, 'title' | 'footer'> {
 ...
 * ```tsx
 * // 基本用法 / Basic usage
 * <SynonModal visible={true} onCancel={handleClose} header="标题">
 *   内容
 * </SynonModal>
 *
 * // 预设尺寸 / Preset size
 * <SynonModal visible={true} size="large" header="大型弹窗">
 *   内容
 * </SynonModal>
 *
...
const SynonModal: React.FC<SynonModalProps> = ({
  children,
  size,
  header,
  footer,
  ...
  const footerConfig: ModalFooterConfig | null = React.useMemo(() => {
    ...
    if (footer === undefined) {
      ...
      const okLabel = props.okText ?? t('common.confirm');
      return {
        render: () => (
  ...
  return (
    ...
    >
      <div className='synon-ai-modal-wrapper' style={{ borderRadius: borderRadiusVal }}>
        {renderHeader()}
        <div className='synon-ai-modal-body-content' style={bodyInlineStyle}>
          {children}
        </div>
        {renderFooter()}
      </div>
...
SynonModal.displayName = 'SynonModal';

export default SynonModal;
