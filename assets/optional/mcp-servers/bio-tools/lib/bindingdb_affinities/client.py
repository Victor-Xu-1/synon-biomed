    ...
    def get_json_root(self, path: str, params: dict) -> dict:
        ...
        for attempt in range(self.max_retries + 1):
            ...
            if not body:
                ...
                # BindingDB occasionally returns a transient empty 200, so it
                # must consume the same bounded retry budget as 5xx/transport
                # failures instead of terminating the scientific task on the
                # first malformed response.
                last_err = BindingDbApiError(
