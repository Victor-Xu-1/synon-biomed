...
timeout above 30 minutes so it isn't a back door to the job surface. `terminate()` any
...
- `references/envs_reference.md` — the Synon Biomed envs as worked examples of the spec: base, pip_phases with the *why*, weight mechanism, the `WORKLOADS[name]` witness command, gotchas. For any provider shape, this is the recipe — the spec fields are what you render (to a Dockerfile, a provider's image API, an Apptainer def file) or run by hand (direct conda/SSH).
