...
describe('VoiceInputSection', () => {
  ...
  it('marks every model as batch transcription because the canonical endpoint is /api/stt', async () => {
    configStore.value = {
      enabled: true,
      provider: 'openai',
      openai: { api_key: 'k', base_url: '', model: 'gpt-4o-transcribe', language: '' },
    ...
    await waitFor(() => expect(screen.queryByText('settings.speechToTextStreamingBadge')).toBeNull());
