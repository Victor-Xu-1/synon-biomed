...
# Keep a single ChEMBL request inside the MCP operation budget. Two bounded
# attempts tolerate a transient edge failure without spending several minutes
# retrying an upstream outage that the agent can route around.
...
class ChemblDrugSearchClient:
    ...
    def paginate(
        ...
        query["offset"] = max(0, int(start_offset))
    ...
    def search_drugs_by_indication(
        ...
        parent_filter: Iterable[str] | None = None,
        start_offset: int = 0,
    ) -> dict[str, Any]:
        ...
        allowed_parents = None if parent_filter is None else {
            str(parent).strip() for parent in parent_filter if str(parent).strip()
        }
        phase_by_parent: dict[str, float] = {}
        for r in rows:
            p = r.get("parent_molecule_chembl_id")
            if not p or (allowed_parents is not None and p not in allowed_parents):
                continue
        ...
        parents = (all_parents[start:] if max_drugs is None
                   else all_parents[start:start + max_drugs])
        molecules = {
        ...
        return {
            "indication_query": {
                ...
                "parent_filter_applied": allowed_parents is not None,
            },
